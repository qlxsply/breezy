...
- 标签栏溢出时显示左右滚动按钮，每次按可视宽度的约 70% 翻页，并将滚动位置对齐到标签边界。
- 标签支持拖动排序；右键标签或使用标签栏操作按钮可关闭当前、其它、左侧、右侧或全部标签。
- 后台最多同时打开 20 个标签，达到上限后必须先关闭已有标签，不允许静默淘汰仍打开的页面状态。
- `USER_ADMIN_TAB_KEEP_ALIVE` 控制是否记忆标签页状态。开启时，未关闭标签的页面实例、输入、弹窗和滚动位置保持不变；关闭时，切换标签会重新挂载页面。
- 当前标签刷新只重建当前页面实例；关闭标签会释放对应页面状态。浏览器刷新不恢复标签会话，只保留当前 URL 对应标签。
...
- 枚举候选项来源约定：统一通过数据字典获取；Boolean 配置使用稳定的 `true/false` 值
...
## web-admin 结构性重构方案（实施中）

### 一、需求背景

`web-admin` 已完成 React 19 + Next.js 16 技术栈迁移，但现有目录、路由、请求、状态、类型和样式仍较多沿用旧 SPA 的组织方式。当前实现已出现后台页面全量同步导入、路由定义重复、全局业务 CSS、registry 职责膨胀、分页请求逻辑重复、401 会话结束不完整及运行时配置安全边界不足等结构性问题。

项目仍处于新项目阶段，本次重构不保留旧前端路径、旧客户端路由分发、旧 Token 存储方式、旧样式选择器和旧 API 组织方式的兼容层。

### 二、当前现状

- 源码主要按 `api / types / core / components / styles` 技术角色组织，同一业务功能分散在多个顶层目录。
- `src/components` 同时包含纯 UI、业务页面、路由壳体和运行时组件。
- 后台业务页面通过 catch-all 路由和客户端 `pageMap` 分发，无法充分利用 Next 路由级代码分割。
- 普通 API 已有统一 fetch 封装，但文件请求和 Bootstrap 请求存在旁路，401 不会完整清理前端会话和运行时连接。
- Token 存储于 localStorage，Bootstrap 地址可以被浏览器查询参数和本地配置覆盖。
- 业务 CSS 全量进入全局作用域，列表页重复维护分页、筛选、loading 和请求生命周期。
- 依赖、测试、类型检查、静态预览和 CI 门禁尚不完整。

### 三、目标

- 将源码重构为 `app / features / shared / runtime / styles`，按业务功能形成垂直切片。
- 每个后台页面使用真实 App Router 物理路由，删除 catch-all 客户端页面分发和重复路由清单。
- 建立统一安全 Cookie 会话、HTTP Transport、401 会话结束和可信运行时配置。
- 将 Store、Service 和 Runtime Adapter 分离，集中权限与认证事实来源。
- 统一后台分页查询、查询表单和分页栏，消除请求竞态和页面样板代码。
- Feature 和组件样式使用 CSS Modules，全局样式只保留 tokens、reset、字体和明确 utility。
- 建立依赖审计、格式、lint、typecheck、测试、构建、E2E 和 bundle budget 门禁。

### 四、技术方案

- 目标目录和依赖方向统一为 `app -> features -> shared`，运行时由 `app -> runtime -> features/shared` 组合。
- 公共 Bz UI 和 Admin UI 迁入 `shared/ui`，不得依赖业务 API、认证 registry 或业务类型。
- 页面、API Payload、业务模型、业务组件和页面样式迁入对应 `features/<feature>`。
- 后端资源树继续驱动菜单和权限，但前端页面必须是编译期存在的真实 Next 路由。
- 所有 HTTP、Blob 和 Bootstrap 请求统一进入支持超时、取消、结构化错误和多响应模式的 Transport。
- 管理后台认证目标为 HttpOnly 安全 Cookie，不保留 localStorage Bearer Token 回退。
- 部署使用受校验的 `/runtime-config.json`，不允许查询参数或 localStorage 修改认证请求 Origin。
- 详细问题矩阵、目标目录、任务范围、验收标准和不兼容原则见 `web-admin/docs/web-admin-architecture-refactoring-plan.md`。

### 五、任务拆分

| 任务编号 | 任务内容 | 状态 |
| --- | --- | --- |
| T1 | 工具链、依赖和质量命令基线 | 已完成 |
| T2 | 可信运行时配置与静态部署重构 | 已完成 |
| T3 | 认证会话与统一 HTTP Transport 重构 | 已完成 |
| T4 | 共享层、公共 UI 和依赖边界重构 | 已完成 |
...
- 不保留双路由、双 API、双 Store、双样式或双认证机制。
