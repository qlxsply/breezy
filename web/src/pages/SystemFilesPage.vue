...
const canAdmin = computed(() => hasResourceCodeAccess("system-file-admin-view"));
const canPhysical = computed(() => hasResourceCodeAccess("system-file-physical-view"));
const canReverse = computed(() => hasResourceCodeAccess("system-file-reference-view"));
const canView = computed(() => hasResourceCodeAccess("system-file-preview"));
const canDownload = computed(() => hasResourceCodeAccess("system-file-download"));
...
async function previewFile(row: SystemFileItem) {
  if (!canView.value || row.type !== "FILE") return;
  clearPreview();
...
async function downloadFile(row: SystemFileItem) {
  if (!canDownload.value || row.type !== "FILE") return;
  try {
...
async function loadPhysicalDetail(item: SystemFileItem) {
  if (!canPhysical.value || item.type !== "FILE") return;
...
async function loadReverseRefs() {
  if (!canReverse.value || !physicalDetail.value) return;
  ...
  if (item.type === "FILE") {
    ...
    if (canPhysical.value) {
      actions.push({
        key: "physical",
        label: "物理信息",
        handler: () => loadPhysicalDetail(item),
      });
    }
  }
  actions.push({
