...
import { TableInput } from "@admin/components/admin-inputs";
import { BzButton } from "@admin/components/bz/BzButton";
import { BzLoading } from "@admin/components/bz/BzLoading";
...
export function AdminDrawerPagedSelection<T>({
  ...
  return (
    <section
      className={`role-manage-section admin-drawer-paged-selection${editable ? " is-editable" : ""}`}
    >
      <div className="role-manage-section__head">
        <div className="role-manage-section__title">{title}</div>
      </div>
        ...
        <div className="admin-grid-table__viewport">
          ...
          >
            <div className="admin-grid-table__body">
              {rows.length === 0 ? (
                <div className="permission-empty">{loading ? "加载中..." : emptyText}</div>
              ) : (
                rows.map((row) => {
