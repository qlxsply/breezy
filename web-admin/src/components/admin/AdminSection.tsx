  ...
  return (
    <section className={["admin-section", className].filter(Boolean).join(" ")}>
      <div className={["admin-section__head", headClassName].filter(Boolean).join(" ")}>
        ...
        {right ? <div className={["admin-section__right", rightClassName].filter(Boolean).join(" ")}>{right}</div> : null}
      </div>
      <div className={["admin-section__body", bodyClassName].filter(Boolean).join(" ")}>{children}</div>
    </section>
  );
}
