...
export function AdminEntityDrawer({ open, title, width = "960px", loading = false, children, extra, footer, onClose }: AdminEntityDrawerProps) {
  if (!open) return null;

  return (
    <div className="admin-entity-drawer">
      <button className="admin-entity-drawer__mask" type="button" aria-label="关闭抽屉" onClick={onClose} />
      <section className="admin-entity-drawer__panel" style={{ width }}>
        <header className="admin-entity-drawer__header">
          ...
          <button className="admin-entity-drawer__close" type="button" aria-label="关闭抽屉" onClick={onClose}>
