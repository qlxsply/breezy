...
export function TableDateTimeInput({
  ...
  const [draft, setDraft] = useState<DateTimeParts>(
    () => parseCanonical(value) || currentZonedParts(),
  );
  ...
  const [inputText, setInputText] = useState(() => formatDateTimeInputValue(value));
  ...
  useEffect(() => {
    if (open) return;
    setInputText(formatDateTimeInputValue(value));
  ...
  function openEditor() {
    ...
    const typed = parseFormatted(inputText, pattern);
    const initial = clampParts(
      typed || parseCanonical(value) || currentZonedParts(),
      minParts,
      maxParts,
    );
    setDraft(initial);
  ...
  function commit(next = draft) {
    ...
    setDraft(normalized);
    setInputText(formatDateTimeInputValue(canonical));
    setInvalid(false);
    setOpen(false);
  ...
  function commitTyped() {
    ...
    const parsed = parseFormatted(inputText, pattern);
  ...
  function selectNow() {
    const now = clampParts(normalizeParts(currentZonedParts(), precision), minParts, maxParts);
    setDraft(now);
...
function parseFormatted(text: string, format: string): DateTimeParts | null {
  ...
  const result = currentZonedParts();
