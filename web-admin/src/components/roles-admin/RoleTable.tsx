...
export function RoleTable({ rows, loading, canEdit, canDelete, canPermissions, onEdit, onPermissions, onRemove }: RoleTableProps) {
  function getActions(role: RoleEntry) {
    ...
    if (canPermissions) {
      actions.push({ key: "permissions", label: "授权", tone: "detail", handler: () => onPermissions(role) });
    }
    if (canDelete) {
  ...
  const columns: Array<BzTableColumn<RoleEntry>> = [
    { key: "code", title: "编码", width: 160, render: (row) => <span className="mono">{row.code}</span> },
    { key: "name", title: "名称", minWidth: 180, render: (row) => <div className="name">{row.name}</div> },
    {
