    ...
    <BzDialog
      ...
      footer={(
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <BzButton buttonType="primary" onClick={handleSubmit}>{mode === "create" ? "创建" : "保存"}</BzButton>
        </div>
      )}
    >
      <div className="role-form-shell">
        <BzForm>
          <div className="role-form-grid">
            <BzFormItem label="编码 *">
              <BzInput modelValue={form.code} placeholder="例如：admin" className="mono" onValueChange={(v) => setForm((prev) => ({ ...prev, code: v }))} />
            </BzFormItem>
            <BzFormItem label="名称 *">
              <BzInput modelValue={form.name} placeholder="例如：管理员" onValueChange={(v) => setForm((prev) => ({ ...prev, name: v }))} />
            </BzFormItem>
          </div>
          <BzFormItem label="启用状态">
            <BzSwitch modelValue={form.enabled} activeText="启用" inactiveText="停用" onValueChange={(v) => setForm((prev) => ({ ...prev, enabled: v }))} />
          </BzFormItem>
        </BzForm>
      </div>
