...
import { markAllRead, markRead, useUnreadCount, useUnreadList } from "@admin/core/registry/notifications-registry";
import Image from "next/image";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import { useEffect, useMemo, useState } from "react";

import { getCurrentRouteTitle, getVisitedTabs, useAdminBreadcrumb, useAdminMenuSections } from "./admin-routes";
  ...
  return (
    <div className={`admin-shell${sidebarCollapsed ? " collapsed" : ""}`}>
      <aside className="admin-sidebar">
        <div className="brand">
          <button className="brand-link" type="button" aria-label="Breezy Admin" onClick={() => router.push("/admin")}>
            <span className="brand-logo">
              <Image src={brandLogo} alt="Breezy Admin" priority />
            </span>
        ...
        <nav className="admin-nav" aria-label="后台导航">
          {sections.map((section) => (
            <div className="nav-node" key={section.id}>
        ...
        <div className="admin-fixed-header">
          <header className="topbar">
              ...
              <nav className="breadcrumb" aria-label="面包屑导航">
                <ol>
                  {breadcrumbItems.map((item, index) => (
                    <li className="breadcrumb-entry" key={`${item.label}-${index}`}>
                      {index > 0 ? (
                      ...
                      {item.href ? (
                        ...
                        <span className={index === breadcrumbItems.length - 1 ? "breadcrumb-current" : "breadcrumb-link"}>
                          {item.label}
                        </span>
                      )}
                    </li>
            ...
            <div className="topbar-right">
              <button className="search-trigger" type="button" title="搜索">
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
              ...
              <Link className="topbar-icon round" href="/admin/profile/preferences" title="设置">
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
              ...
              <div className="topbar-popover-host">
                ...
                {notificationOpen ? (
                  <section className="notification-panel">
                    ...
                    <button className="mark-read" type="button" onClick={() => void markAllRead()}>
                      全部标记为已读
                    </button>
                    <div className="notification-list">
                      {previewList.length === 0 ? <div className="notification-empty">暂无未读消息</div> : null}
                      {previewList.map((item) => (
                        <button key={item.id} className="notification-item" type="button" onClick={() => void markRead(item.id)}>
                    ...
                    <footer className="panel-footer">
                      ...
                      <button className="primary" type="button" onClick={() => setNotificationOpen(false)}>
                        查看全部
                      </button>
                    </footer>
                  </section>
          ...
          <div className="tabs-row">
            <div className="tabs-scroll">
              <div className="tabs-list">
                {visitedTabs.map((tab) => {
                  ...
                  return (
                    <Link key={tab.href} href={tab.href} className={`tab-item${active ? " active" : ""}${tab.pinned ? " pinned" : ""}`}>
            ...
            <div className="tabs-tools">
              <button className="tabbar-tool" type="button" title="刷新当前页" onClick={() => router.refresh()}>
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
      ...
      <div className="admin-shell-status" aria-live="polite">
        当前页面：{currentTitle}
      </div>
    </div>
  );
