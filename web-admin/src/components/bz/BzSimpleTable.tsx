...
export function BzSimpleTable<Row>({
  ...
  return (
    <BzLoading loading={loading} className="bz-simple-table__loading-wrap">
      <div className="bz-simple-table">
        <table className="bz-simple-table__inner">
          ...
          {data.length > 0 ? (
                ...
                <tr key={String(resolveRowKey(row, rowIndex))}>
                  {columns.map((column) => (
                    <td
                      ...
                      className={column.align ? `is-${column.align}` : undefined}
                    >
                      {column.render ? column.render(row, rowIndex) : renderDefaultCell(row, rowIndex, column)}
                    </td>
                  ))}
                </tr>
              ))}
            ...
            <tbody>
              <tr>
                <td className="bz-simple-table__empty" colSpan={Math.max(columns.length, 1)}>
                  {emptyText}
                </td>
              </tr>
            </tbody>
