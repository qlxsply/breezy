...
export function AdminDateTimeRangeField({
  ...
  const displayText = useMemo(() => {
    const startLabel = formatDateTimeInputValue(startValue, dateTimePattern, timeZone);
    const endLabel = formatDateTimeInputValue(endValue, dateTimePattern, timeZone);
    if (startLabel && endLabel) return `${startLabel}–${endLabel}`;
    if (startLabel) return `${startLabel} 起`;
    if (endLabel) return `截至 ${endLabel}`;
    return "";
  }, [dateTimePattern, endValue, startValue, timeZone]);
