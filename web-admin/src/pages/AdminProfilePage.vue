          ...
          <div class="profile-grid">
            <section class="profile-section">
              <div class="profile-section__head">
                ...
                <div class="profile-section__actions">
                  <bz-button v-if="editingBasic" :disabled="basicSaving" @click="cancelBasicEdit">取消</bz-button>
                  <bz-button
              ...
              <div v-if="profile" class="profile-info-grid">
                ...
                <div class="profile-info-item">
                  ...
                  <bz-input v-if="editingBasic" v-model="basicForm.nickname" placeholder="请输入昵称" />
                  <span v-else class="profile-info-item__value">{{ profile.nickname || "-" }}</span>
                </div>
                <div class="profile-info-item">
                  <span class="profile-info-item__label">类型</span>
                  <span class="profile-info-item__value">{{ userTypeLabel(profile.userType) }}</span>
                </div>
                <div class="profile-info-item">
                ...
                <div class="profile-info-item">
                  ...
                  <span class="profile-info-item__value">{{ formatDateTime(profile.lastPasswordChangedAt) }}</span>
                </div>
                <div class="profile-info-item">
                  <span class="profile-info-item__label">最近更新时间</span>
                  <span class="profile-info-item__value">{{ formatDateTime(profile.updatedAt) }}</span>
                </div>
              </div>
            </section>
            ...
            <section class="profile-section profile-section--wide">
              ...
              <div class="admin-table-surface">
                <bz-table :data="activityPage.elements" size="small" empty-text="暂无记录">
                  <bz-table-column label="事件" width="120">
                    <template #default="scope">{{ activityTypeLabel(scope.row.eventType) }}</template>
                  </bz-table-column>
                  <bz-table-column label="结果" width="100">
                    <template #default="scope">
                      <bz-tag :type="scope.row.success ? 'success' : 'danger'">{{ scope.row.success ? "成功" : "失败" }}</bz-tag>
                    </template>
                  </bz-table-column>
                  <bz-table-column prop="loginIp" label="IP" min-width="140" show-overflow-tooltip>
                    <template #default="scope">{{ scope.row.loginIp || "-" }}</template>
                  </bz-table-column>
                  <bz-table-column prop="remark" label="备注" min-width="220" show-overflow-tooltip>
                    <template #default="scope">{{ scope.row.remark || "-" }}</template>
                  </bz-table-column>
                  <bz-table-column label="发生时间" width="180">
                    <template #default="scope">{{ formatDateTime(scope.row.occurredAt) }}</template>
                  </bz-table-column>
                </bz-table>
              </div>

              <div v-if="activityPage.totalElements > 0" class="dict-pagination-bar profile-pagination">
                <div class="dict-pagination-summary">共 {{ activityPage.totalElements }} 条记录</div>
                <div class="dict-pagination-right">
                  <label class="dict-page-size">
                    <select class="dict-page-size__select" :value="activityPageSize" @change="handleActivityPageSizeSelect">
                      <option v-for="size in pageSizeOptions" :key="size" :value="size">{{ size }}条/页</option>
                    </select>
                  </label>
                  <div class="dict-page-list">
                    <button class="dict-page-btn dict-page-btn--icon" type="button" :disabled="activityIsFirstPage" @click="goToActivityPage(1)"><span aria-hidden="true">|&lt;</span></button>
                    <button class="dict-page-btn dict-page-btn--icon" type="button" :disabled="activityIsFirstPage" @click="goToActivityPage(activityPageNo - 1)"><span aria-hidden="true">&lt;</span></button>
                    <template v-for="(token, tokenIndex) in activityPageTokens" :key="`${String(token)}-${tokenIndex}`">
                      <button v-if="typeof token === 'number'" class="dict-page-btn" :class="{ 'is-active': token === activityPageNo }" type="button" @click="goToActivityPage(token)">{{ token }}</button>
                      <span v-else class="dict-page-ellipsis">...</span>
                    </template>
                    <button class="dict-page-btn dict-page-btn--icon" type="button" :disabled="activityIsLastPage" @click="goToActivityPage(activityPageNo + 1)"><span aria-hidden="true">&gt;</span></button>
                    <button class="dict-page-btn dict-page-btn--icon" type="button" :disabled="activityIsLastPage" @click="goToActivityPage(activityTotalPages)"><span aria-hidden="true">&gt;|</span></button>
                  </div>
                </div>
              </div>
            </section>
...
function handleActivityPageSizeSelect(event: Event) {
  ...
  if (!Number.isFinite(nextPageSize) || nextPageSize <= 0 || nextPageSize === activityPageSize.value) return;
...
.profile-section { border: 1px solid #e5e7eb; border-radius: 16px; background: #fff; padding: 18px; display: grid; gap: 14px; }
.profile-section--wide { grid-column: 1 / -1; }
.profile-section__head { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.profile-section__title { font-size: 15px; font-weight: 800; color: #0f172a; }
.profile-section__actions { display: flex; gap: 8px; }
.profile-info-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px 16px; }
.profile-info-item { display: grid; gap: 6px; }
.profile-info-item__label { color: #64748b; font-size: 12px; font-weight: 700; }
.profile-info-item__value { color: #0f172a; font-size: 14px; font-weight: 600; word-break: break-all; }
.dict-pagination-bar { display:flex; align-items:center; justify-content:space-between; gap:14px; flex-wrap:wrap; padding:14px 0 0; color:#475569; }
