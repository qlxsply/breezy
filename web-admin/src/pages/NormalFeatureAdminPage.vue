        ...
        >
          <template #header>
            <div class="admin-table-header">
              <div class="admin-table-title">功能配置</div>
        ...
        >
          ...
          >
            <div class="detail-field">
              <span class="detail-field__label">功能编码</span
              ><span class="detail-field__value">{{ detail.code }}</span>
            </div>
            <div class="detail-field">
              <span class="detail-field__label">名称</span
              ><span class="detail-field__value">{{ detail.name }}</span>
            </div>
            <div class="detail-field">
              <span class="detail-field__label">状态</span
              ><span class="detail-field__value">{{ detail.enabled ? "启用" : "停用" }}</span>
            </div>
            <div class="detail-field">
              <span class="detail-field__label">权限数</span
              ><span class="detail-field__value">{{ detail.permissionCodes.length }}</span>
            </div>
            <div class="detail-field detail-field--wide">
              <span class="detail-field__label">描述</span
              ><span class="detail-field__value">{{ detail.description || "-" }}</span>
            </div>
            <div class="detail-field detail-field--wide">
              <span class="detail-field__label">权限码</span
...
const page = ref<PageResult<NormalFeatureEntry>>({
  pageNo: 1,
  pageSize: 10,
  numberOfElements: 0,
  totalPages: 0,
...
const detail = ref<NormalFeatureEntry | null>(null);
...
function getRowActions(row: NormalFeatureEntry): AdminActionItem[] {
...
async function openDetail(id: string) {
  ...
  try {
    detail.value = await getNormalFeature(id);
  } finally {
    detailLoading.value = false;
  }
}
async function toggleStatus(row: NormalFeatureEntry) {
  if (!canToggle.value) return;
  const nextEnabled = !row.enabled;
  const confirmed = await bzConfirm({
    title: nextEnabled ? "启用功能" : "停用功能",
