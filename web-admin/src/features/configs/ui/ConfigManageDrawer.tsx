...
import { BzDragHandle } from "@admin/shared/ui/bz/BzDragHandle";
import { BzIconActionButton } from "@admin/shared/ui/bz/BzIconActionButton";
import { BzSwitch } from "@admin/shared/ui/bz/BzSwitch";
import { BzTag } from "@admin/shared/ui/bz/BzTag";
import type { DragEvent } from "react";
import { useEffect, useState } from "react";

import styles from "./ConfigManageDrawer.module.css";
  ...
  useEffect(() => {
    ...
    void Promise.all(
      Object.entries(USER_PREFERENCE_DICT_CODES).map(async ([path, code]) => {
        const items = await listPublicDictOptions(code, { signal: controller.signal });
        return [
          path,
...
function TimeOffsetEditor({
  ...
  const { dateTimePattern, precision, timeZone } = useDateTimePreferences();
  ...
  const formatterVersion = `${dateTimePattern}:${timeZone}`;
  useEffect(() => {
