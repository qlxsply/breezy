...
public class BootstrapSystemFileSyncService {
  ...
  public BootstrapTaskReport run(boolean dryRun) {
    ...
    String message =
        "total="
            ...
            + summary.skipped()
            + "; ownerType="
            + OwnerType.APPLICATION.name()
            + "; ownerId="
            + DefaultUser.SYSTEM.account();
    return new BootstrapTaskReport(
        BootstrapTaskKey.SYSTEM_FILE_SYNC,
        dryRun,
        true,
  ...
  private FolderSyncSummary inspect(Connection connection, List<FolderSeed> seeds)
      throws SQLException {
  ...
  private FolderSyncSummary synchronize(Connection connection, List<FolderSeed> seeds)
      throws SQLException {
    Set<String> existingFolderNames = loadExistingFolderNames(connection);
    int created = 0;
    int skipped = 0;
  ...
  private record FolderSeed(String id, String ownerType, String ownerId, String folderName) {}

  private record FolderSyncSummary(int created, int skipped) {}
}
