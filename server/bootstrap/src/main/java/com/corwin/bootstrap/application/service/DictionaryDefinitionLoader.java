...
public class DictionaryDefinitionLoader {
        ...
        try {
            ...
            try (InputStream is = xmlResource.getInputStream()) {
                ...
                for (DictionaryNode node : nodes) {
                    result.add(new DictTypeSeed(node.getId(), node.getCode(), node.getName(), node.getDescription(),
                            node.getEnumClass(), node.getValueType(), node.getStructureType(),
                            Boolean.TRUE.equals(node.getEnabled()), flattenEnumDictItems(node)));
                }
                return result;
            }
        } catch (Exception e) {
    ...
    private void checkDictUniqueness(List<DictionaryNode> nodes) {
        ...
        for (DictionaryNode node : nodes) {
            ...
            if (!dictCodes.add(node.getCode())) {
                throw new IllegalStateException("Duplicate dictionary CODE found in XML: " + node.getCode());
            }
            if (!StrUtil.isNotBlank(node.getEnumClass())) {
                throw new IllegalStateException("Dictionary enumClass must not be blank: " + node.getCode());
            }
        }
    }
    ...
    private List<DictItemSeed> flattenEnumDictItems(DictionaryNode node) throws Exception {
        ...
        if (!DictEnumDefinition.class.isAssignableFrom(enumClass)) {
            throw new IllegalStateException("enumClass must implement DictEnumDefinition: " + node.getEnumClass());
        }
        ...
        for (int i = 0; i < constants.length; i++) {
            ...
            if (!itemCodes.add(itemCode)) {
                throw new IllegalStateException(
                        "Duplicate dictionary item CODE found in enum " + node.getEnumClass() + ": " + itemCode);
            }
            result.add(new DictItemSeed(node.getCode() + ":" + itemCode, null, itemCode, definition.label(),
                    definition.itemValue(), i + 1, true, i == 0, blankToNull(definition.tagColor()),
                    blankToNull(definition.tagType()), null, null));
        }
        return result;
    }
    ...
    private void validateXml(Resource xmlResource, Resource xsdResource) throws Exception {
        ...
        try (InputStream xsdInput = xsdResource.getInputStream(); InputStream xmlInput = xmlResource.getInputStream()) {
