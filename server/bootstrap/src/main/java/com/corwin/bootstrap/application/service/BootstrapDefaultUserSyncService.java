...
public class BootstrapDefaultUserSyncService {
    ...
    public BootstrapTaskReport run(boolean dryRun) {
        ...
        UserSyncSummary summary = dryRun ? BootstrapJdbcTransactionSupport.execute(dataSource,
        ...
        String message = "total=" + seeds.size() + "; created=" + summary.created() + "; skipped=" + summary.skipped() + "; adminPassword=" + DEFAULT_ADMIN_PASSWORD;
        return new BootstrapTaskReport(BootstrapTaskKey.DEFAULT_USER_SYNC, dryRun, true,
    ...
    private UserSyncSummary synchronize(Connection connection, List<UserSeed> seeds) throws SQLException {
        Set<Long> existingIds = loadExistingIds(connection);
        int created = 0;
        int skipped = 0;
        for (UserSeed seed : seeds) {
    ...
    private List<UserSeed> buildSeeds() {
        ...
        for (DefaultUser defaultUser : DefaultUser.values()) {
            String rawPassword = defaultUser == DefaultUser.ADMIN ? DEFAULT_ADMIN_PASSWORD : randomSystemPassword();
            result.add(new UserSeed(defaultUser.id(), defaultUser.account(), defaultUser.account(),
                    defaultUser.userType().name(), BCrypt.hashpw(rawPassword, BCrypt.gensalt()), PASSWORD_ALGO,
                    UserStatus.ENABLED.name(), false, OPERATOR));
        }
    ...
    private record UserSyncSummary(
            int created,
            int skipped
    ) {
    }
