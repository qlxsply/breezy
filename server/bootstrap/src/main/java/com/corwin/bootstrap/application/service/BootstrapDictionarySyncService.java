...
public class BootstrapDictionarySyncService {
  ...
  public BootstrapTaskReport run(boolean dryRun) {
    ...
    List<DictTypeSeed> definitions =
        dictionaryDefinitionLoader.loadDefinitions(
    ...
    String message =
        "dictTypes="
            ...
            + "; dictItems="
            + itemRows.size()
            + "; strategy=delete-all-and-rebuild";
    return new BootstrapTaskReport(
        BootstrapTaskKey.DICTIONARY_SYNC,
        dryRun,
        true,
  ...
  private void batchInsertTypes(Connection connection, List<DictTypeSeed> definitions)
      throws SQLException {
  ...
  private void bindTypeParameters(PreparedStatement ps, DictTypeSeed definition)
      throws SQLException {
  ...
  private record DictItemRow(String dictTypeId, DictItemSeed item) {}
}
