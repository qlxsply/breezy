 ...
 */
public class BootstrapDatabasePreflightInitializer implements ApplicationContextInitializer<ConfigurableApplicationContext> {
    ...
    public void initialize(ConfigurableApplicationContext applicationContext) {
        ...
        environment.getPropertySources().addFirst(new MapPropertySource("bootstrapConsoleDataSource", overrides));
    }
    ...
    private void ensureDatabaseExists(DataSourceSettings settings) {
        String databaseName = BootstrapJdbcUrlSupport.extractDatabaseName(settings.databaseType(), settings.url());
        ...
        try {
            ...
            String adminUrl = BootstrapJdbcUrlSupport.buildAdminUrl(settings.databaseType(), settings.url());
            boolean exists = BootstrapJdbcTransactionSupport.execute(
    ...
    private String loadCreateDatabaseSql(DatabaseType databaseType, String databaseName) throws Exception {
        String path = "bootstarp/sql/" + BootstrapJdbcUrlSupport.toFolder(databaseType) + "/create_database.sql";
        try (var inputStream = Thread.currentThread().getContextClassLoader().getResourceAsStream(path)) {
            ...
            return template.replace("${databaseName}",
                    BootstrapJdbcUrlSupport.quoteIdentifier(databaseType, databaseName));
        }
    }
    ...
    private DatabaseType selectDatabaseType(Scanner scanner) {
        while (true) {
            ...
            if ("2".equals(input) || "pgsql".equalsIgnoreCase(input) || "postgresql".equalsIgnoreCase(input)) {
    ...
    private String buildJdbcUrl(DatabaseType databaseType, String host, int port, String databaseName) {
        return switch (databaseType) {
            case MYSQL ->
                    "jdbc:mysql://" + host + ":" + port + "/" + databaseName + "?serverTimezone=UTC&rewriteBatchedStatements=true&useSSL=false&allowPublicKeyRetrieval=true";
