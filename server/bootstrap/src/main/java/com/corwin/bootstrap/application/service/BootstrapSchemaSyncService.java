...
public class BootstrapSchemaSyncService {
    ...
    private static final Pattern CREATE_INDEX_PATTERN = Pattern.compile(
            "(?is)create\\s+(unique\\s+)?index\\s+([`\"\\[]?[A-Za-z0-9_.$]+[`\"\\]]?)\\s+on\\s+" + "([`\"\\[]?[A-Za-z0-9_.$]+[`\"\\]]?)\\s*\\(([^;]+?)\\)\\s*;");
    ...
    public BootstrapTaskReport run(boolean dryRun) {
        ...
        List<BootstrapSqlTemplateService.ScriptResource> scripts = sqlTemplateService.listSchemaScripts();
        ...
        if (expectedTables.isEmpty()) {
            return new BootstrapTaskReport(BootstrapTaskKey.SCHEMA_SYNC, dryRun, true,
        ...
        try {
            SchemaInspectionResult inspectionResult = BootstrapJdbcTransactionSupport.execute(dataSource,
                    connection -> inspectSchema(connection, expectedTables));
            ...
            for (SchemaAction action : inspectionResult.actions()) {
                if (action.type() == SchemaActionType.CREATE) {
                    ...
                    if (!dryRun) {
                        BootstrapJdbcTransactionSupport.executeWithoutResult(dataSource,
                ...
                boolean recreate = consolePrompt.confirm(
                        "表 " + action.tableName() + " 结构不一致，是否删除旧表并重新创建？", false);
                if (!recreate) {
            ...
            return new BootstrapTaskReport(BootstrapTaskKey.SCHEMA_SYNC, dryRun, true,
                    System.currentTimeMillis() - startedAt, summary.render(dryRun));
        } catch (Exception e) {
    ...
    private SchemaInspectionResult inspectSchema(Connection connection,
            Map<String, TableDefinition> expectedTables) throws Exception {
        ...
        for (TableDefinition expected : expectedTables.values()) {
            ...
            if (actualTableName == null) {
                ...
                actions.add(new SchemaAction(SchemaActionType.CREATE, expected.name(), null, expected.createStatement(),
                        List.of()));
                continue;
            }
            ...
            actions.add(new SchemaAction(SchemaActionType.RECREATE, expected.name(), actualTableName,
                    expected.createStatement(), mismatches));
        }
    ...
    private List<TableDefinition> parseTables(BootstrapSqlTemplateService.ScriptResource script) {
        ...
        for (int i = 0; i < matches.size(); i++) {
            ...
            result.add(new TableDefinition(normalizeIdentifier(current.rawTableName()), parseColumns(inner),
                    parseIndexes(block, current.rawTableName()), block));
        }
        return result;
    }
    ...
    private List<ColumnDefinition> parseColumns(String inner) {
        ...
        for (String segment : segments) {
            ...
            columns.add(new ColumnDefinition(normalizeIdentifier(rawName), normalizedType, nullable, order++));
        }
        return columns;
    }

    private List<String> compareTable(Connection connection, DatabaseMetaData metaData, TableDefinition expected,
            String actualTableName) throws Exception {
    ...
    private List<String> compareColumns(Connection connection, DatabaseMetaData metaData, TableDefinition expected,
            String actualTableName) throws Exception {
        Map<String, ActualColumn> actualColumns = loadActualColumns(connection, metaData, actualTableName);
        List<String> mismatches = new ArrayList<>();
        ...
        for (ColumnDefinition column : expected.columns()) {
            ...
            if (actual == null) {
                mismatches.add("missing column " + column.name() + ", expected type " + column.normalizedType());
                continue;
            }
            if (!isSameColumnType(column.normalizedType(), actual.normalizedType())) {
                mismatches.add(
                        "column " + column.name() + " type mismatch, expected=" + column.normalizedType() + ", actual=" + actual.normalizedType());
            }
            if (column.nullable() != actual.nullable()) {
                mismatches.add(
                        "column " + column.name() + " nullability mismatch, expected=" + (column.nullable() ? "NULL" : "NOT NULL") + ", actual=" + (actual.nullable() ? "NULL" : "NOT NULL"));
            }
            if (column.order() != actual.order()) {
                mismatches.add(
                        "column " + column.name() + " order mismatch, expected=" + column.order() + ", actual=" + actual.order());
            }
        }
        for (ActualColumn actual : actualColumns.values()) {
            if (!expectedByName.containsKey(actual.lookupName())) {
                mismatches.add("unexpected column " + actual.name() + ", actual type " + actual.normalizedType());
            }
        }
        return mismatches;
    }
    ...
    private List<String> compareIndexes(Connection connection, DatabaseMetaData metaData, TableDefinition expected,
            String actualTableName) throws Exception {
        Map<String, ActualIndex> actualIndexes = loadActualIndexes(connection, metaData, actualTableName);
        List<String> mismatches = new ArrayList<>();
        for (IndexDefinition expectedIndex : expected.indexes()) {
            ...
            if (expectedIndex.unique() != actualIndex.unique()) {
                mismatches.add(
                        "index " + expectedIndex.name() + " uniqueness mismatch, expected=" + expectedIndex.unique() + ", actual=" + actualIndex.unique());
            }
            if (!Objects.equals(expectedIndex.lookupColumns(), actualIndex.lookupColumns())) {
                mismatches.add(
                        "index " + expectedIndex.name() + " columns mismatch, expected=" + expectedIndex.columns() + ", actual=" + actualIndex.columns());
            }
        }
        return mismatches;
    }

    private Map<String, String> loadActualTables(Connection connection, DatabaseMetaData metaData) throws Exception {
    ...
    private Map<String, ActualColumn> loadActualColumns(Connection connection, DatabaseMetaData metaData,
        ...
        try (ResultSet rs = metaData.getColumns(catalog, schema, tableName, "%")) {
            while (rs.next()) {
                ...
                result.put(normalizeLookup(name), new ActualColumn(name,
                        normalizeActualType(rs.getInt("DATA_TYPE"), rs.getString("TYPE_NAME"), rs.getInt("COLUMN_SIZE"),
                                rs.getInt("DECIMAL_DIGITS")), rs.getInt("NULLABLE") != DatabaseMetaData.columnNoNulls,
    ...
    private Map<String, ActualIndex> loadActualIndexes(Connection connection, DatabaseMetaData metaData,
            String tableName) throws Exception {
        String catalog = connection.getCatalog();
        String schema = connection.getSchema();
    ...
    private void loadActualIndexes(DatabaseMetaData metaData, String catalog, String schema, String tableName,
            Map<String, ActualIndexBuilder> builders) throws Exception {
        try (ResultSet rs = metaData.getIndexInfo(catalog, schema, tableName, false, false)) {
            while (rs.next()) {
                ...
                ActualIndexBuilder builder = builders.computeIfAbsent(normalizeLookup(indexName),
                        key -> new ActualIndexBuilder(indexName, unique));
                builder.addColumn(rs.getShort("ORDINAL_POSITION"), columnName);
            }
        }
    }

    private void dropTable(Connection connection, DatabaseType databaseType, String tableName) throws SQLException {
        String sql = "drop table if exists " + BootstrapJdbcUrlSupport.quoteIdentifier(databaseType, tableName);
    ...
    private void executeSql(Connection connection, String sql, String scriptName) {
        ScriptUtils.executeSqlScript(connection,
                new ByteArrayResource(sql.getBytes(StandardCharsets.UTF_8), scriptName));
    }
    ...
    private boolean isConstraintDefinition(String definition) {
        ...
        return normalized.startsWith("primary key") || normalized.startsWith("unique") || normalized.startsWith(
                "constraint") || normalized.startsWith("index") || normalized.startsWith(
                "key") || normalized.startsWith("foreign key");
    }
    ...
    private String extractTypeExpression(String definition) {
        ...
        for (String token : tokens) {
            ...
            if (lower.equals("not") || lower.equals("null") || lower.equals("default") || lower.equals(
                    "comment") || lower.equals("constraint") || lower.equals("primary") || lower.equals(
                    "unique") || lower.equals("references") || lower.equals("check") || lower.equals(
                    "auto_increment") || lower.equals("identity") || lower.equals("generated") || lower.equals(
                    "always") || lower.equals("as")) {
                break;
            }
            collected.add(token);
        }
    ...
    private String normalizeExpectedType(String typeExpression) {
        String normalized = Objects.requireNonNull(normalizeLookup(typeExpression)).replaceAll("\\s+", "");
        return normalized.replace("unsigned", "");
    }

    private String normalizeActualType(int jdbcType, String typeName, int size, int scale) {
        return switch (jdbcType) {
            case Types.VARCHAR, Types.NVARCHAR, Types.CHAR, Types.NCHAR, Types.LONGVARCHAR, Types.LONGNVARCHAR ->
                    normalizeLookup(typeName) + "(" + size + ")";
            case Types.DECIMAL, Types.NUMERIC -> normalizeLookup(typeName) + "(" + size + "," + scale + ")";
            case Types.TIMESTAMP, Types.TIMESTAMP_WITH_TIMEZONE, Types.DATE, Types.TIME, Types.TIME_WITH_TIMEZONE,
                 Types.BIGINT, Types.INTEGER, Types.SMALLINT, Types.TINYINT, Types.BOOLEAN, Types.BIT, Types.CLOB,
                 Types.BLOB, Types.BINARY, Types.VARBINARY, Types.LONGVARBINARY, Types.DOUBLE, Types.FLOAT,
                 Types.REAL -> normalizeLookup(typeName);
    ...
    private static final class ActualIndexBuilder {
        ...
        private ActualIndex build() {
            List<String> columns = orderedColumns.entrySet().stream().sorted(Map.Entry.comparingByKey())
    ...
    private static final class SchemaDiffSummary {
        ...
        private String render(boolean dryRun) {
            ...
            return String.join("; ", parts) + (details.isEmpty() ? "" : " | " + String.join(" | ", details));
        }
    }

    private record SchemaInspectionResult(
