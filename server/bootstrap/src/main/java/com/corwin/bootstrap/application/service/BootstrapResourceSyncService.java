...
public class BootstrapResourceSyncService {
  ...
  public BootstrapTaskReport run(boolean dryRun) {
    ...
    xmlValidationService.validate(
        definitionResources.resourcesXml(), definitionResources.resourcesXsd());
    xmlValidationService.validate(
        definitionResources.userFeaturesXml(), definitionResources.userFeaturesXsd());

    List<BootstrapResourceDefinitionLoader.ResourceSeed> resources =
        resourceDefinitionLoader.loadDefinitions(definitionResources.resourcesXml());
    BootstrapUserFeatureDefinitionLoader.UserFeatureDefinitionSeed userFeatures =
        userFeatureDefinitionLoader.loadDefinitions(definitionResources.userFeaturesXml());
    ResourceStats expectedStats = summarize(resources, userFeatures);

    ResourceStats stats =
        dryRun
            ? validateOnly(resources, userFeatures, expectedStats)
            : BootstrapJdbcTransactionSupport.execute(
                dataSource, connection -> rebuild(connection, resources, userFeatures));
    ...
    return new BootstrapTaskReport(
        BootstrapTaskKey.RESOURCE_SYNC,
        dryRun,
        true,
  ...
  private ResourceStats validateOnly(
      List<BootstrapResourceDefinitionLoader.ResourceSeed> resources,
      BootstrapUserFeatureDefinitionLoader.UserFeatureDefinitionSeed userFeatures,
      ResourceStats expectedStats) {
    return BootstrapJdbcTransactionSupport.execute(
        dataSource,
        connection -> {
          Map<String, PermissionRef> permissionByCode =
              loadPermissionByCode(
                  connection, sqlTemplateService.load("resource_select_permission_code_id.sql"));
          validatePermissionReferences(permissionByCode, resources, userFeatures);
          return expectedStats;
        });
  }

  private ResourceStats rebuild(
  ...
  private void validatePermissionReferences(
    ...
    for (BootstrapUserFeatureDefinitionLoader.ApplicationSeed application :
        userFeatures.applications()) {
    ...
    for (String permissionCode : internalPermissionCodes) {
      ...
      if (permission == null) {
        throw new IllegalStateException(
            "resource permission missing in sys_permission: " + permissionCode);
      }
      if (permission.userScope() != UserType.ADMIN) {
        throw new IllegalStateException(
            "resource permission must be ADMIN-only: " + permissionCode);
      }
    }
    for (String permissionCode : webUserPermissionCodes) {
      ...
      if (permission.userScope() != UserType.USER) {
        throw new IllegalStateException(
            "user feature permission must be USER-only: " + permissionCode);
      }
    }
  }

  private void collectInternalPermissionCodes(
      BootstrapResourceDefinitionLoader.ResourceSeed resource, Set<String> collector) {
  ...
  private void insertResourceTree(
      Connection connection,
      Long parentId,
      BootstrapResourceDefinitionLoader.ResourceSeed resource,
      Map<String, PermissionRef> permissionByCode,
      ResourceStats stats)
      throws SQLException {
  ...
  private void upsertUserFeatures(
      Connection connection,
  ...
  private Long insertResource(
      Connection connection, Long parentId, BootstrapResourceDefinitionLoader.ResourceSeed resource)
      throws SQLException {
  ...
  private Long upsertApplication(
      Connection connection, BootstrapUserFeatureDefinitionLoader.ApplicationSeed seed)
      throws SQLException {
    ...
    return selectIdByCode(
        connection, "user_feature_select_application_id_by_code.sql", seed.code(), "application");
  }

  private Long upsertFeature(
      Connection connection,
      Long applicationId,
      BootstrapUserFeatureDefinitionLoader.FeatureSeed seed)
      throws SQLException {
    try (PreparedStatement ps =
        connection.prepareStatement(
  ...
  private Long selectIdByCode(Connection connection, String sqlName, String code, String type)
      throws SQLException {
  ...
  private Long selectFeatureId(Connection connection, Long applicationId, String code)
      throws SQLException {
    try (PreparedStatement ps =
        connection.prepareStatement(
            sqlTemplateService.load(
                "user_feature_select_feature_id_by_application_and_code.sql"))) {
  ...
  private void insertFeaturePermissionBinding(
      Connection connection, Long applicationId, Long featureId, Long permissionId)
      throws SQLException {
    try (PreparedStatement ps =
        connection.prepareStatement(
  ...
  private Map<String, PermissionRef> loadPermissionByCode(Connection connection, String sql)
      throws SQLException {
  ...
  private Long readGeneratedKey(PreparedStatement ps, String type, String code)
      throws SQLException {
  ...
  private ResourceStats summarize(
      List<BootstrapResourceDefinitionLoader.ResourceSeed> resources,
      BootstrapUserFeatureDefinitionLoader.UserFeatureDefinitionSeed userFeatures) {
    ResourceStats stats = new ResourceStats();
    for (BootstrapResourceDefinitionLoader.ResourceSeed resource : resources) {
  ...
  private void summarizeResource(
      BootstrapResourceDefinitionLoader.ResourceSeed resource, ResourceStats stats) {
    stats.resourceCount++;
    if (resource.resourceType().canBindPermission()) {
  ...
  private record PermissionRef(Long id, UserType userScope) {}
