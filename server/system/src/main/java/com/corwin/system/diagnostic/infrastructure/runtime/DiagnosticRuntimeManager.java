...
/**
 * Central manager for the diagnostic runtime lifecycle. Orchestrates session start/stop, snapshot
 * scheduling, collector coordination, and automatic expiry handling.
 *
...
public class DiagnosticRuntimeManager {
  ...
  public DiagnosticRuntimeManager(
  ...
  /**
   * Starts a new diagnostic session: configures repositories, clears previous state, starts JFR if
   * enabled, and begins periodic snapshot capture.
   *
  ...
  public synchronized DiagnosticSession start(DiagnosticSession session) {
    ...
    ScheduledFuture<?> future =
        diagnosticScheduler.scheduleAtFixedRate(
            () -> captureSnapshot(session), session.config().intervalMs());
  ...
  /**
   * Updates an existing diagnostic session with new config: reschedules snapshot tasks and restarts
   * JFR if the configuration changed.
   *
  ...
  /**
   * Checks whether the given diagnostic item is currently being collected. Automatically stops the
  ...
  private record RuntimeState(DiagnosticSession session, ScheduledFuture<?> future) {}
}
