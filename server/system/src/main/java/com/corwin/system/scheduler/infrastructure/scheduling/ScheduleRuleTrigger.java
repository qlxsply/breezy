...
/**
 * Spring {@link Trigger} implementation that computes next execution times from a {@link
 * ScheduleRule}.
 *
...
public class ScheduleRuleTrigger implements Trigger {
  ...
  @Nullable
  public static Instant nextExecutionAfter(
      ScheduleRule rule,
  ...
  @Nullable
  private static Instant nextExecution(
      ScheduleRule rule,
      @Nullable Instant lastScheduledExecution,
      @Nullable Instant lastCompletion,
      Instant now) {
    return switch (rule) {
      ...
      case FixedIntervalRule fixedIntervalRule ->
          nextFixedInterval(fixedIntervalRule, lastScheduledExecution, lastCompletion, now);
  ...
  private static Instant nextOnce(
      OnceRule rule, @Nullable Instant lastScheduledExecution, Instant now) {
  ...
  private static Instant nextFixedInterval(
      FixedIntervalRule rule,
      @Nullable Instant lastScheduledExecution,
      @Nullable Instant lastCompletion,
      Instant now) {
    ...
    Instant nextBase =
        rule.mode() == IntervalMode.FIXED_RATE
  ...
  private static Instant toMonthlyInstant(MonthlyRule rule, YearMonth yearMonth, ZoneId zoneId) {
    int day =
        rule.dayMode() == MonthlyDayMode.LAST_DAY
