 ...
 * <p>Provides read-only access to enabled dictionary data for internal use and external consumers.</p>
 *
...
public class DictQueryService {
    ...
    public List<DictItemView> listEnabledItems(String code) {
        ...
        return dictItemRepository.findByDictTypeIdAndEnabledTrueOrderBySortNoAscItemLabelAsc(dictType.getId()).stream()
    ...
    public boolean existsValue(String code, String value) {
        DictType dictType = getEnabledType(code);
        return dictItemRepository.findByDictTypeIdAndItemValue(dictType.getId(), value).filter(DictItem::isEnabled)
                .isPresent();
    }
    ...
    public boolean existsCode(String code, String itemCode) {
        DictType dictType = getEnabledType(code);
        return dictItemRepository.findByDictTypeIdAndItemCode(dictType.getId(), itemCode)
    ...
    public String resolveLabel(String code, String value) {
        DictType dictType = getEnabledType(code);
        return dictItemRepository.findByDictTypeIdAndItemValue(dictType.getId(), value).filter(DictItem::isEnabled)
                .map(DictItem::getItemLabel).orElse(null);
    }

    private DictType getEnabledType(String code) {
        BizAssert.notBlank(code, BaseError.MISSING_PARAMETER);
        DictType dictType = dictTypeRepository.findByCode(code.trim())
