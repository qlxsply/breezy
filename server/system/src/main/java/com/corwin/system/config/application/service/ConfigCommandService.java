...
public class ConfigCommandService {
  ...
  public ConfigChangeView update(UpdateConfigCommand command) {
    ...
    if (current == null) {
      ...
      savedValue =
          current.updateOverride(
              prepared.content(), spec.schemaVersion(), updatedBy, updatedAt, command.reason());
    ...
    for (ResetConfigCommand command : commands) {
      if (!configKeys.add(command.configKey())) {
        throw new BizException(
            "Duplicate config key: " + command.configKey(), BaseError.INVALID_PARAMETER);
      }
    }
  ...
  private ConfigChangeView resetDefaultValue(ResetConfigCommand command) {
    ...
    ConfigValue resetValue =
        current.resetToDefault(
            ConfigJsonCodec.serialize(spec.defaultValue()),
            spec.schemaVersion(),
            currentUserId(),
            HighDate.realInstant(),
            command.reason());
  ...
  private <T> PreparedConfig<T> prepare(
      ConfigSpec<T> spec, JsonNode submitted, ConfigValue current) {
    ...
    ConfigValueTreeSupport.MergeResult merged =
        ConfigValueTreeSupport.mergeSensitive(
            submitted, currentTree.value(), currentTree.readable(), spec.fields());
    ...
    try {
      ...
      violations.add(
          new ConfigViolation("", "INVALID_TYPE", "Config value has an invalid structure"));
  ...
  private void requireValid(ConfigSpec<?> spec, PreparedConfig<?> prepared) {
    if (!prepared.violations().isEmpty()) {
      throw new ConfigValidationException(
          "Config publish validation failed: " + spec.key(), prepared.violations());
    }
  }
  ...
  private ConfigSpec<?> requireEditableSpec(String configKey) {
    ConfigSpec<?> spec =
        ConfigSpecCatalog.find(configKey)
            .orElseThrow(
                () ->
                    new BizException(
                        "Config definition not found: " + configKey, BaseError.NOT_FOUND));
  ...
  private <T> void refreshAfterCommit(ConfigSpec<T> spec, ConfigValue savedValue) {
    ...
    ConfigSnapshot<T> snapshot =
        ConfigSnapshotFactory.createStored(
            spec,
            savedValue.content(),
            savedValue.schemaVersion(),
            savedValue.revision(),
            savedValue.configured());
  ...
  private record PreparedConfig<T>(T value, String content, List<ConfigViolation> violations) {
  ...
  private record CurrentTree(JsonNode value, boolean readable) {}
}
