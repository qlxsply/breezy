...
/**
 * REST controller for scheduler job administration (list, detail, pause, resume, cancel, trigger,
 * delete).
 *
...
public class SchedulerAdminController {
  ...
  @Authorize(
      ...
      permissions = {"scheduler.job.view"})
  public ApiResponse<List<SchedulerJobRes>> list() {
    return ApiResponse.ok(
        queryAppService.listJobs().stream().map(SchedulerAdminController::toRes).toList());
  }
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"scheduler.job.view"})
  public ApiResponse<PageData<SchedulerJobExecutionRes>> executions(
      @PathVariable String jobId, @RequestBody SchedulerJobExecutionPageReq req) {
    PageData<SchedulerJobExecutionView> page =
        queryAppService.pageExecutions(jobId, PageSpecFactory.of(req.page(), null));
    return ApiResponse.ok(
        PageData.of(
            page.pageNo(),
            page.pageSize(),
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"scheduler.job.resume"})
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"scheduler.job.cancel"})
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"scheduler.job.trigger"})
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"scheduler.job.delete"})
  ...
  private static SchedulerJobRes toRes(SchedulerJobView view) {
    return new SchedulerJobRes(
        view.jobId(),
        view.namespace(),
        view.name(),
        view.jobType(),
        view.handlerKey(),
        view.scheduleRuleType(),
        view.enabled(),
        view.allowConcurrent(),
        view.remark(),
        view.status(),
        view.nextFireTime(),
        view.lastFireTime(),
        view.lastSuccessTime(),
        view.lastFailureTime(),
        view.lastErrorMessage(),
        view.updatedAt());
  }

  private static SchedulerJobDetailRes toDetailRes(SchedulerJobDetailView view) {
    return new SchedulerJobDetailRes(
        view.jobId(),
        view.namespace(),
        view.name(),
        view.source(),
