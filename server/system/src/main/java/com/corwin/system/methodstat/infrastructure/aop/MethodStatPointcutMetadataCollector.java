...
/**
 * Implementation of {@link MethodStatMetadataCollector} that scans all beans in the application
 * context and registers metadata for methods matching the configured pointcut.
 *
...
public class MethodStatPointcutMetadataCollector implements MethodStatMetadataCollector {
  ...
  /**
   * Iterate over all registered beans, match methods against the pointcut, and register their
  ...
  private Method resolveInvocableMethod(Class<?> targetClass, Method candidateMethod) {
    ...
    try {
      ...
      log.debug(
          "Method stat metadata collect skipped missing method, class={}, method={}",
          targetClass.getName(),
          candidateMethod);
      return null;
    }
  ...
  private void addDescriptor(
      Map<String, MethodStatMethodDescriptor> descriptors,
      Method declaredMethod,
      Class<?> targetClass) {
    try {
      MethodStatMethodDescriptor descriptor =
          annotationResolver.resolve(declaredMethod, targetClass);
