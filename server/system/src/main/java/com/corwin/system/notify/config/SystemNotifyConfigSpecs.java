...
public final class SystemNotifyConfigSpecs {
  ...
  public static final ConfigSpec<SseTicketConfig> SSE =
      new SimpleConfigSpec<>(
          new ConfigKey("system.notify.sse"),
          "system",
          "notify",
          "SSE Ticket",
          "SSE 一次性 ticket 有效时间",
          SseTicketConfig.class,
          new SseTicketConfig(60),
          1,
          10,
          ConfigActivationPolicy.DYNAMIC,
          ConfigEditPolicy.ADMIN_EDITABLE,
          ConfigInvalidValuePolicy.USE_DEFAULT,
          List.of(optional("ttlSeconds", "有效秒数", ConfigFieldType.LONG, 10)),
          "default",
          SystemNotifyConfigSpecs::validateSse,
          SystemNotifyConfigSpecs::validateSse);

  public static final ConfigSpec<WebPushVapidConfig> WEB_PUSH =
      new SimpleConfigSpec<>(
          new ConfigKey("system.notify.web-push"),
          "system",
          "notify",
          "Web Push",
          "Web Push VAPID 公私钥和联系主体",
          WebPushVapidConfig.class,
          new WebPushVapidConfig("", "", "mailto:breezy@localhost"),
          1,
          20,
          ConfigActivationPolicy.DYNAMIC,
          ConfigEditPolicy.ADMIN_EDITABLE,
          ...
          List.of(
              optional("publicKey", "VAPID 公钥", ConfigFieldType.STRING, 10),
              optionalSensitive("privateKey", "VAPID 私钥", ConfigFieldType.STRING, 20),
              optional("subject", "VAPID 联系主体", ConfigFieldType.STRING, 30)),
          "web-push-vapid",
          SystemNotifyConfigSpecs::validateWebPush,
          SystemNotifyConfigSpecs::validateWebPush);

  public static final ConfigSpec<MessageTypeConfigs> MESSAGE_TYPES =
      new SimpleConfigSpec<>(
          new ConfigKey("system.notify.message-types"),
          "system",
          "notify",
          "消息类型策略",
          "不同消息类型的通道、路由和优先级策略",
          MessageTypeConfigs.class,
          new MessageTypeConfigs(
              List.of(
                  new MessageTypeConfig(
                      ...
                      false),
                  new MessageTypeConfig(
                      MsgType.SYSTEM_EVENT, "", MsgPriority.LOW, true, false, false, false),
                  new MessageTypeConfig(
                      MsgType.BUSINESS_EVENT, "", MsgPriority.MEDIUM, true, false, true, false))),
          1,
          30,
          ConfigActivationPolicy.DYNAMIC,
          ConfigEditPolicy.ADMIN_EDITABLE,
          ConfigInvalidValuePolicy.USE_DEFAULT,
          List.of(optional("items", "消息类型配置", ConfigFieldType.OBJECT, 10)),
          "message-type-config",
  ...
  private static List<ConfigViolation> validateWebPush(WebPushVapidConfig value) {
    ...
    if (hasPublic
        && (!validBase64(value.publicKey(), 65) || !validBase64(value.privateKey(), 32))) {
    ...
    if (value.subject() == null || value.subject().isBlank() || !validSubject(value.subject())) {
      violations.add(
          new ConfigViolation("subject", "INVALID_URI", "VAPID 联系主体必须是 mailto 或 https URI"));
    }
    return violations;
  }
