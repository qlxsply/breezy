...
public final class SystemNotifyConfigSpecs {
    ...
    public static final ConfigSpec<SseTicketConfig> SSE = new SimpleConfigSpec<>(
            new ConfigKey("system.notify.sse"), "system", "notify", "SSE Ticket",
            "SSE 一次性 ticket 有效时间", SseTicketConfig.class, new SseTicketConfig(60),
            1, 10, ConfigActivationPolicy.DYNAMIC, ConfigEditPolicy.ADMIN_EDITABLE,
            ConfigInvalidValuePolicy.USE_DEFAULT,
            List.of(field("ttlSeconds", "有效秒数", ConfigFieldType.LONG, false, 10)), "default",
            SystemNotifyConfigSpecs::validateSse, SystemNotifyConfigSpecs::validateSse
    );

    public static final ConfigSpec<WebPushVapidConfig> WEB_PUSH = new SimpleConfigSpec<>(
            new ConfigKey("system.notify.web-push"), "system", "notify", "Web Push",
            "Web Push VAPID 公私钥和联系主体", WebPushVapidConfig.class,
            new WebPushVapidConfig("", "", "mailto:breezy@localhost"), 1, 20,
            ConfigActivationPolicy.DYNAMIC, ConfigEditPolicy.ADMIN_EDITABLE,
            ConfigInvalidValuePolicy.FAIL_STARTUP,
            List.of(
                    field("publicKey", "VAPID 公钥", ConfigFieldType.STRING, false, 10),
                    field("privateKey", "VAPID 私钥", ConfigFieldType.STRING, true, 20),
                    field("subject", "VAPID 联系主体", ConfigFieldType.STRING, false, 30)
            ), "web-push-vapid", SystemNotifyConfigSpecs::validateWebPush,
            SystemNotifyConfigSpecs::validateWebPush
    );
    ...
    public static final ConfigSpec<MessageTypeConfigs> MESSAGE_TYPES = new SimpleConfigSpec<>(
            ...
            "不同消息类型的通道、路由和优先级策略", MessageTypeConfigs.class,
            new MessageTypeConfigs(List.of(
                    new MessageTypeConfig(MsgType.TODO_REMINDER, "/todo/all", MsgPriority.MEDIUM,
                            true, false, true, false),
                    new MessageTypeConfig(MsgType.SYSTEM_EVENT, "", MsgPriority.LOW,
                            true, false, false, false),
                    new MessageTypeConfig(MsgType.BUSINESS_EVENT, "", MsgPriority.MEDIUM,
                            true, false, true, false)
            )), 1, 30, ConfigActivationPolicy.DYNAMIC, ConfigEditPolicy.ADMIN_EDITABLE,
            ConfigInvalidValuePolicy.USE_DEFAULT,
            List.of(field("items", "消息类型配置", ConfigFieldType.OBJECT, false, 10)),
            "message-type-config", SystemNotifyConfigSpecs::validateMessageTypes,
            SystemNotifyConfigSpecs::validateMessageTypes
    );

    public record SseTicketConfig(long ttlSeconds) {
    }

    public record WebPushVapidConfig(String publicKey, String privateKey, String subject) {
    }
    ...
    private static List<ConfigViolation> validateSse(SseTicketConfig value) {
        return value.ttlSeconds() >= 10
                ? List.of()
                : List.of(new ConfigViolation("ttlSeconds", "OUT_OF_RANGE", "SSE ticket 有效时间不能少于 10 秒"));
    }
    ...
    private static List<ConfigViolation> validateMessageTypes(MessageTypeConfigs value) {
        ...
        for (int index = 0; index < value.items().size(); index++) {
            ...
            if (item.route() != null && !item.route().isBlank() && !item.route().startsWith("/")) {
                violations.add(new ConfigViolation("items[" + index + "].route", "INVALID_ROUTE", "消息路由必须以 / 开头"));
            }
        }
    ...
    private static ConfigFieldSpec field(
        ...
        return new ConfigFieldSpec(path, title, "", type, false, sensitive, false, order, "",
