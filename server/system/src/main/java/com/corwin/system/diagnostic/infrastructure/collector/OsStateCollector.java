...
/**
 * Collector that captures OS-level state including host info, CPU/memory usage, network interfaces,
 * and disks.
 *
...
public class OsStateCollector {
  ...
  public OsSnapshot collect() {
    ...
    return new OsSnapshot(
        ...
        baseBean.getVersion(),
        baseBean.getArch(),
        baseBean.getAvailableProcessors(),
        ...
        osBean == null ? -1L : osBean.getFreeSwapSpaceSize(),
        collectDisks());
  }
  ...
  private List<NetworkInterfaceSnapshot> collectNetworkInterfaces() {
    try {
      ...
      return Collections.list(networkInterfaces).stream()
          .map(this::toNetworkInterfaceSnapshot)
          .toList();
    } catch (Exception ex) {
      return List.of();
    }
  ...
  private NetworkInterfaceSnapshot toNetworkInterfaceSnapshot(NetworkInterface networkInterface) {
    ...
    try {
      ...
      return new NetworkInterfaceSnapshot(
          networkInterface.getName(), networkInterface.getDisplayName(), false, false, addresses);
    }
  }
