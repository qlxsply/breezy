...
/**
 * Application service for diagnostic query operations: retrieving session status, snapshots,
 * events, and runtime capabilities.
 *
...
public class DiagnosticQueryAppService {
  ...
  public DiagnosticQueryAppService(
  ...
  public DiagnosticSessionView status() {
    ...
    return runtimeManager
        ...
        .orElseGet(
            () ->
                new DiagnosticSessionView(
                    DiagnosticStatus.INACTIVE, DiagnosticConfig.defaultConfig(), null, null, 0L));
  }
  ...
  /**
   * Returns the diagnostic capabilities available in the current runtime environment, including
   * JFR, HTTP, connection pool, and SQL monitoring availability.
   *
   * @return the diagnostic capability view
   */
  public DiagnosticCapabilityView capabilities() {
    return new DiagnosticCapabilityView(
        jfrCollector.isAvailable(),
        true,
        dbPoolStateCollector.hasRegisteredDataSource(),
        dbPoolStateCollector.hasRegisteredDataSource(),
        true,
        dbPoolStateCollector.dataSourceNames());
  }
