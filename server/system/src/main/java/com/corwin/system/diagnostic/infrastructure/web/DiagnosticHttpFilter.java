...
/**
 * Servlet filter that intercepts HTTP requests to collect request metrics (duration, status,
 * errors) and emit diagnostic events when the HTTP diagnostic item is enabled.
 *
...
public class DiagnosticHttpFilter extends OncePerRequestFilter implements Ordered {
  ...
  public DiagnosticHttpFilter(
      DiagnosticRuntimeManager runtimeManager, HttpRequestObserver httpRequestObserver) {
    this.runtimeManager = runtimeManager;
    this.httpRequestObserver = httpRequestObserver;
  }

  /**
   * Measures request duration and reports metrics to HttpRequestObserver when HTTP monitoring is
   * enabled.
   */
  @Override
  protected void doFilterInternal(
    ...
    try {
      ...
      httpRequestObserver.onRequestCompleted(
          request.getMethod(),
          request.getRequestURI(),
          response.getStatus(),
          durationMs,
          error,
          runtimeManager.slowRequestThresholdMs());
    }
  }
