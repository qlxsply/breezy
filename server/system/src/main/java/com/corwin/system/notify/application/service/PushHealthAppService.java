...
public class PushHealthAppService {
  ...
  /**
   * Queries the push health status for the specified user, including subscription details, delivery
   * info, and VAPID readiness.
   *
  ...
  public PushHealthView queryHealth(Long userId, UserType userType) {
    ...
    List<UserPushSubscription> subscriptions =
        userPushSubscriptionRepository.findByUserIdAndUserTypeOrderByUpdatedAtDesc(
            userId, userType);
    ...
    Instant latestUpdatedAt =
        subscriptions.stream()
            .map(UserPushSubscription::getUpdatedAt)
            .filter(it -> it != null)
            .max(Comparator.naturalOrder())
            .orElse(null);
    Instant latestPushAt =
        subscriptions.stream()
            .map(UserPushSubscription::getLastPushAt)
            .filter(it -> it != null)
            .max(Comparator.naturalOrder())
            .orElse(null);
    String latestError =
        subscriptions.stream()
            .map(UserPushSubscription::getLastError)
            .filter(it -> it != null && !it.isBlank())
            .findFirst()
            .orElse("");

    List<PushHealthSubscriptionView> subscriptionViews =
        subscriptions.stream().map(this::toSubscriptionView).toList();

    PushHealthDeliveryView latestDelivery =
        messageDeliveryRepository
            .findFirstByUserIdAndUserTypeOrderByIdDesc(userId, userType)
  ...
  public PushHealthDeliveryView sendHighPriorityHealthCheck(Long userId, UserType userType) {
    ...
    MessageDelivery delivery =
        notificationDispatcher.dispatchPreview(
            userId,
            userType,
            MsgType.SYSTEM_EVENT,
            "[健康检查] 高优先级系统通知测试",
            "如果你收到了这条系统通知，说明 Web Push 投递链路正常。",
            "/todo/all",
            "HIGH",
            false,
            true,
            ...
            true);
    log.info(
        "Push health test message dispatched: userType={}, userId={}, deliveryId={}",
        userType,
        userId,
        delivery.getId());
  ...
  private PushHealthSubscriptionView toSubscriptionView(UserPushSubscription subscription) {
    return new PushHealthSubscriptionView(
        subscription.getDeviceId(),
        subscription.isActive(),
        resolveEndpointHost(subscription.getEndpoint()),
        subscription.getUpdatedAt(),
        subscription.getLastPushAt(),
        subscription.getLastError());
  }

  private PushHealthDeliveryView toDeliveryView(MessageDelivery delivery) {
    return new PushHealthDeliveryView(
