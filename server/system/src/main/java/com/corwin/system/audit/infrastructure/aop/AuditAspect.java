...
public class AuditAspect {
  ...
  public AuditAspect(
      AuditAnnotationResolver annotationResolver,
      ...
      auditLogService.record(
          new AuditRecordCommand(
              ...
              operation.recordRequest()
                  ? payloadSanitizer.summarizeRequestParameters(
                      request == null ? null : request.getParameterMap(), auditPolicy)
                  : null,
              operation.recordRequest()
                  ? payloadSanitizer.summarizeRequestBody(joinPoint.getArgs(), auditPolicy)
                  : null,
              success && operation.recordResponse()
                  ? payloadSanitizer.summarizeResponseBody(result, auditPolicy)
                  : null,
              success,
              resolveErrorCode(throwable),
              throwable == null
                  ? null
                  : payloadSanitizer.summarizeErrorMessage(throwable, auditPolicy),
              startedAt,
              endedAt,
              durationMs,
              endedAt));
