...
public class TaskSchedulerManager {
  ...
  /**
   * Schedules a task with the given cron expression. Cancels any existing schedule for the same
   * code.
   *
    ...
    try {
      ...
      ScheduledFuture<?> future =
          taskScheduler.schedule(
              () -> wrapAndExecute(code, bean, method), new CronTrigger(cronExpr));
  ...
  private void wrapAndExecute(String code, Object bean, Method method) {
    ...
    try {
      ...
      CtxUtil.setPrincipal(
          new AuthPrincipal(
              schedulerUser.id(),
              schedulerUser.account(),
              schedulerUser.userType(),
              false,
              Set.of()));
