...
/**
 * Adapter implementing {@link AuditLogRepository} by delegating to JPA for CRUD operations and
 * MyBatis for paginated dynamic queries. Normalizes query string fields with LIKE patterns before
 * execution.
 *
...
public class AuditLogRepositoryJpaAdapter implements AuditLogRepository {
  ...
  /**
   * Executes a paginated dynamic query via the MyBatis mapper. String filter fields are normalized
   * to LIKE patterns before execution.
   *
  ...
  private AuditLogPageQuery normalizeQuery(AuditLogPageQuery query) {
    ...
    return new AuditLogPageQuery(
        LikePatternUtils.toContainsPattern(query.traceId()),
        query.operatorUserId(),
        LikePatternUtils.toContainsPattern(query.operatorUsername()),
        ...
        LikePatternUtils.toContainsPattern(query.auditAction()),
        query.auditLevel(),
        query.success(),
        query.startAt(),
        query.endAt());
