 ...
 *
 * <p>Delegates basic CRUD to {@link ApiJpaRepository} and paginated queries to {@link
 * ApiMybatisMapper} for dynamic filtering support.
 *
...
public class ApiRepositoryJpaAdapter implements ApiRepository {
  ...
  @Override
  public Optional<Api> findFirstByProtocolAndHttpMethodAndPathPattern(
  ...
  private ApiPageQuery normalizeQuery(ApiPageQuery query) {
    ...
    return new ApiPageQuery(
        StrUtil.trimToNull(query.module()),
        LikePatternUtils.toContainsPattern(query.pathPattern()),
        LikePatternUtils.toContainsPattern(query.handlerClass()),
        LikePatternUtils.toContainsPattern(query.handlerMethod()),
