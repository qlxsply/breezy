...
/**
 * REST controller for audit log management. Provides endpoints for paginated listing and
 * single-record retrieval, restricted to ADMIN users with the {@code audit.log.view} permission.
 *
...
public class AuditLogController {
  ...
  private static AuditLogRes toRes(AuditLog auditLog) {
    return new AuditLogRes(
        ...
        auditLog.getOperatorUserId(),
        auditLog.getOperatorUsername(),
        auditLog.getOperatorUserType(),
        auditLog.getApplicationCode(),
        auditLog.getProtocol(),
        auditLog.getHttpMethod(),
        auditLog.getPathPattern(),
        auditLog.getRequestUri(),
        parsePermissionCodes(auditLog.getPermissionCodes()),
        auditLog.getAuditResource(),
        auditLog.getAuditAction(),
        auditLog.getAuditDescription(),
        auditLog.getAuditLevel(),
        auditLog.getRequestIp(),
        auditLog.getUserAgent(),
        auditLog.getRequestParamSummary(),
        auditLog.getRequestBodySummary(),
        auditLog.getResponseSummary(),
        Boolean.TRUE.equals(auditLog.getSuccess()),
        auditLog.getErrorCode(),
        auditLog.getErrorMessage(),
        auditLog.getStartedAt(),
        auditLog.getEndedAt(),
