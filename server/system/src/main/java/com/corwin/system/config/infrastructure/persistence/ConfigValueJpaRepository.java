...
public interface ConfigValueJpaRepository extends JpaRepository<ConfigValueEntity, String> {
    ...
    @Query("""
            ...
            """)
    int updateByExpectedRevision(@Param("configKey") String configKey, @Param("content") String content,
            @Param("schemaVersion") int schemaVersion, @Param("nextRevision") long nextRevision,
            @Param("configured") boolean configured, @Param("updatedBy") Long updatedBy,
            @Param("updatedAt") Instant updatedAt, @Param("updateReason") String updateReason,
