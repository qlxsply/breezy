...
/**
 * Domain model encapsulating the configuration parameters for a diagnostic session. Performs
 * validation and clamping of all numeric fields in the compact constructor.
 *
...
public record DiagnosticConfig(
  ...
  public DiagnosticConfig {
    ...
    historyCapacity =
        clamp(
            historyCapacity, MIN_HISTORY_CAPACITY, MAX_HISTORY_CAPACITY, DEFAULT_HISTORY_CAPACITY);
    eventCapacity =
        clamp(eventCapacity, MIN_EVENT_CAPACITY, MAX_EVENT_CAPACITY, DEFAULT_EVENT_CAPACITY);
    ...
    slowSqlThresholdMs =
        clamp(slowSqlThresholdMs, MIN_THRESHOLD_MS, Long.MAX_VALUE, DEFAULT_SLOW_SQL_THRESHOLD_MS);
  ...
  public static DiagnosticConfig defaultConfig() {
    return new DiagnosticConfig(
        ...
        EnumSet.allOf(DiagnosticItem.class),
        false,
        DEFAULT_SLOW_REQUEST_THRESHOLD_MS,
        DEFAULT_SLOW_SQL_THRESHOLD_MS,
        DEFAULT_TTL_SECONDS);
  }
