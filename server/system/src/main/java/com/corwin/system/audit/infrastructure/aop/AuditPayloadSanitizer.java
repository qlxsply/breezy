...
public class AuditPayloadSanitizer {
  ...
  public String summarizeRequestParameters(
      Map<String, String[]> parameterMap, AuditPolicyConfig policy) {
  ...
  private JsonNode sanitizeNode(JsonNode node, String fieldName) {
    ...
    if (node.isObject()) {
      ...
      node.fields()
          .forEachRemaining(
              entry ->
                  sanitized.set(entry.getKey(), sanitizeNode(entry.getValue(), entry.getKey())));
      return sanitized;
    }
    if (node.isArray()) {
  ...
  private boolean isIgnoredArgument(Object argument) {
    return argument instanceof ServletRequest
        ...
        || argument instanceof RedirectAttributes
  ...
  private boolean isSimpleValue(Object payload) {
    return payload instanceof CharSequence
        ...
        || payload instanceof java.time.temporal.Temporal
        || payload instanceof Date
        || payload instanceof UUID;
  }
