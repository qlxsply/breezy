...
/**
 * Application service for end-user (non-admin) operations including self-registration, profile
 * retrieval and update, password change, and logout.
 *
...
public class UserService {
  ...
  /**
   * Registers a new end-user with username/password credentials, creates identity and credential
   * records, assigns default feature packages, and records the registration lifecycle event.
   *
  ...
  public UserProfileView register(RegisterUserCommand cmd) {
    ...
    BizAssert.state(
        webUserCurrentIdentityRepository
            .findByIdentityTypeAndIdentityHash(WebUserIdentityType.USERNAME, identityHash)
            .isEmpty(),
        BaseError.CONFLICT);
    ...
    WebUser user =
        webUserRepository.save(
            new WebUser(
                nickname,
                nickname,
                WebUserRegisterMethod.USERNAME_PASSWORD,
                WebUserRegisterChannel.WEB,
                "self-register",
                null,
                null,
                username));
    WebUserIdentity identity =
        webUserIdentityRepository.save(
            new WebUserIdentity(
                user.getId(),
                WebUserIdentityType.USERNAME,
                username,
                webUserIdentitySupport.normalizeForType(WebUserIdentityType.USERNAME, username),
                identityHash,
                null,
                null,
                true,
                true,
                username));
    webUserCredentialRepository.save(
        new WebUserCredential(
            user.getId(),
            identity.getId(),
            WebUserCredentialType.PASSWORD,
            BCrypt.hashpw(cmd.password(), BCrypt.gensalt()),
            "bcrypt",
            username));
    webUserCurrentIdentityRepository.save(
        new WebUserCurrentIdentity(
            WebUserIdentityType.USERNAME,
            identityHash,
            null,
            null,
    ...
    webUserLifecycleService.record(
        user.getId(),
        WebUserLifecycleEventType.REGISTERED,
  ...
  public UserProfileView updateMyProfile(UpdateMyProfileCommand cmd) {
    ...
    user = webUserRepository.save(user);
    webUserLifecycleService.record(
        user.getId(), WebUserLifecycleEventType.PROFILE_UPDATED, Map.of());
    return toView(user, resolveAccount(user));
  }
