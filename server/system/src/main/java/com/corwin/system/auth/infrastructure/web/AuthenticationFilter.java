...
/**
 * Servlet filter that authenticates each incoming request by resolving the authorization header
 * (JWT or opaque token) or SSE ticket, then sets the {@link AuthPrincipal} into the request
 * context.
 *
...
public class AuthenticationFilter extends OncePerRequestFilter implements Ordered {
  ...
  private AuthError resolveError(RuntimeException ex) {
    if (ex instanceof BizException bizException
        && bizException.getErrorCode() instanceof AuthError authError) {
      return authError;
    }
