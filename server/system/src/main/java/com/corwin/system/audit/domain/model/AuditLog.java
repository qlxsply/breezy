...
/**
 * JPA entity representing an audit log record. Maps to the {@code sys_audit_log} table and captures
 * the full lifecycle of a single audited request, including operator context, request details,
...
@Table(
    name = "sys_audit_log",
    indexes = {
      @Index(name = "idx_sys_audit_log_trace_id", columnList = "trace_id"),
      ...
      @Index(
          name = "idx_sys_audit_log_path",
          columnList = "application_code,http_method,path_pattern"),
      @Index(
          name = "idx_sys_audit_log_resource_action",
          columnList = "audit_resource,audit_action"),
