...
/**
 * Application service for querying and listing storage nodes. Implements the {@link FileQueryPort}
 * inbound port and provides both owner-scoped and global (admin) query capabilities.
 *
...
public class FileQueryService implements FileQueryPort {
  ...
  /**
   * Lists storage nodes scoped to a specific owner, with optional recursive traversal, keyword
   * filtering, and sorting.
   */
  @Override
  public List<StorageNodeView> listContent(
      OwnerType ownerType, String ownerId, StorageQueryCommand query) {
    List<LogicalFile> nodes = new ArrayList<>();

    collectOwnerScopedNodes(
        toDomainOwnerType(ownerType), ownerId, query.parentId(), query.recursive(), nodes);
  ...
  /**
   * Lists all storage nodes globally (admin scope) with optional recursive traversal, keyword
   * filtering, and sorting.
   */
  ...
  public List<StorageNodeView> listLogicalFileRefs(
      String physicalFileId, String keyword, StorageSortBy sortBy, StorageSortOrder sortOrder) {
  ...
  private void collectOwnerScopedNodes(
      OwnerType ownerType,
      String ownerId,
      String parentId,
      boolean recursive,
    ...
    if (!recursive) {
      nodes.addAll(
          logicalFileRepository.findByOwnerTypeAndOwnerIdAndParentId(ownerType, ownerId, parentId));
      return;
    }
    ...
    while (cursor < pendingParents.size()) {
      ...
      List<LogicalFile> currentNodes =
          logicalFileRepository.findByOwnerTypeAndOwnerIdAndParentId(
              ownerType, ownerId, currentParentId);
      nodes.addAll(currentNodes);
      pendingParents.addAll(
          currentNodes.stream().filter(LogicalFile::isFolder).map(LogicalFile::getId).toList());
    }
  }
  ...
  private List<StorageNodeView> buildNodeViews(List<LogicalFile> nodes, String keyword) {
    ...
    Map<String, PhysicalFile> physicalMap =
        toPhysicalMap(nodes.stream().filter(LogicalFile::isFile).toList());
  ...
  private Map<String, PhysicalFile> toPhysicalMap(List<LogicalFile> files) {
    Set<String> physicalIds =
        files.stream().map(LogicalFile::getPhysicalFileId).collect(Collectors.toSet());
  ...
  private void sortNodes(
      List<StorageNodeView> nodes, StorageSortBy sortBy, StorageSortOrder sortOrder) {
  ...
  private Comparator<StorageNodeView> buildComparator(StorageSortBy sortBy) {
    return switch (sortBy) {
      case SIZE ->
          Comparator.comparingLong(this::sortableSize)
              .thenComparing(node -> normalizeName(node.name()))
              .thenComparing(StorageNodeView::id);
      case TYPE ->
          Comparator.comparingInt(this::typeRank)
              .thenComparing(node -> normalizeName(node.name()))
              .thenComparing(StorageNodeView::id);
      case UPDATED_AT ->
          Comparator.comparing(
                  this::sortableUpdatedAt, Comparator.nullsLast(Comparator.naturalOrder()))
              .thenComparing(node -> normalizeName(node.name()))
              .thenComparing(StorageNodeView::id);
      case NAME ->
          Comparator.comparing((StorageNodeView node) -> normalizeName(node.name()))
              .thenComparing(StorageNodeView::id);
    };
  }
  ...
  /**
   * Resolves the MIME content type for a file based on its extension if the provided content type
   * is null or blank.
   */
