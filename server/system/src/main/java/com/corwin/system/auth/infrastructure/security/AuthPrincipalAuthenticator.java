...
/**
 * Authenticator component that resolves raw tokens (admin opaque, user JWT, or SSE ticket) into
 * {@link AuthPrincipal} objects, with caching and validation against the user and session stores.
 *
...
public class AuthPrincipalAuthenticator {
  ...
  public AuthPrincipalAuthenticator(
      AuthSessionCacheService authSessionCacheService,
  ...
  /**
   * Authenticates an admin (opaque) token by looking up the session hash, validating session state
   * and user status, and returning the cached or freshly-built principal.
   *
  ...
  public AuthPrincipal authenticateAdminToken(String rawToken) {
    ...
    if (session.getSessionStatus() == SessionStatus.REVOKED
        || session.getSessionStatus() == SessionStatus.KICKED_OUT) {
      authSessionCacheService.delete(tokenHash);
    ...
    AuthPrincipal principal =
        new AuthPrincipal(
            ...
            user.getUsername(),
            user.getUserType(),
            DefaultUser.isAdmin(user.getId()),
            user.isMustChangePassword(),
            readPermissionSnapshot(session));
  ...
  /**
   * Authenticates an user JWT token by parsing and validating the web user JWT payload, user
   * status, token version, and restrictions.
   *
  ...
  public AuthPrincipal authenticateUserToken(String rawToken) {
    ...
    if (payload.issuedAt() == null
        || (user.getTokenNotBefore() != null
            && payload.issuedAt().isBefore(user.getTokenNotBefore()))) {
      throw new BizException(AuthError.TOKEN_REVOKED);
    }
    ...
    return new AuthPrincipal(
        user.getId(), payload.account(), UserType.USER, false, permissionCodes);
  }

  /**
   * Authenticates using an SSE ticket payload, building a principal from the ticket's user info and
  ...
  public AuthPrincipal authenticateSseTicket(TokenPayload payload) {
    if (payload == null
        || payload.userId() == null
        || payload.userAccount() == null
        || payload.userAccount().isBlank()) {
      throw new BizException(AuthError.INVALID_TOKEN);
    }
    UserType userType = payload.userType();
    Set<String> permissionCodes =
        userType == null || userType == UserType.GUEST
            ? Set.of()
    ...
    return new AuthPrincipal(
        ...
        payload.userAccount(),
        userType,
        DefaultUser.isAdmin(payload.userId()),
        permissionCodes);
  }
