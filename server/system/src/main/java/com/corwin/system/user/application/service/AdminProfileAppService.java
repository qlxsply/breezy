...
/**
 * Application service for admin profile operations including profile retrieval, update, and login
 * activity queries.
 *
...
public class AdminProfileAppService {
  ...
  /**
   * Returns the profile of the currently authenticated admin user, including recent login activity.
   *
  ...
  private AdminProfileView toView(User user) {
    List<AdminProfileLoginActivityView> recentActivities =
        loginLogService.listRecentLoginActivities(user.getUsername(), 20).stream()
  ...
  private static AdminProfileLoginActivityView toActivityView(LoginEvent event) {
    return new AdminProfileLoginActivityView(
        event.getId(),
        event.getEventType(),
        event.isSuccess(),
        event.getLoginIp(),
  ...
  private User currentInternalUser() {
    ...
    BizAssert.state(
        user.getUserType() == UserType.ADMIN || DefaultUser.isSystemUser(user.getId()),
        BaseError.FORBIDDEN);
    return user;
  }
