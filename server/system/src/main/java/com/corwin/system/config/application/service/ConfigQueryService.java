...
public class ConfigQueryService {
    ...
    public PageData<ConfigManagementView> page(String keyword, String module, String group, PageSpec pageSpec) {
        PageSpec effectivePageSpec = PageSpec.ensure(pageSpec);
        Map<String, ConfigValue> storedValues = configValueRepository.findAll().stream()
        ...
        List<ConfigManagementView> matched = ConfigSpecCatalog.loadAll().values().stream()
                .filter(spec -> matches(spec, normalizedKeyword, normalizedModule, normalizedGroup))
                .sorted(specComparator()).map(spec -> toView(spec, storedValues.get(spec.key().value()))).toList();

        long offset = Math.max(0L, (long) (effectivePageSpec.pageNo() - 1) * effectivePageSpec.pageSize());
    ...
    private boolean matches(ConfigSpec<?> spec, String keyword, String module, String group) {
        ...
        return normalize(spec.key().value()).contains(keyword) || normalize(spec.title()).contains(
                keyword) || normalize(spec.description()).contains(keyword);
    }

    private Comparator<ConfigSpec<?>> specComparator() {
        return Comparator.comparing(ConfigSpec<?>::module).thenComparing(ConfigSpec::group)
    ...
    private <T> ConfigManagementView toView(ConfigSpec<T> spec, ConfigValue storedValue) {
        ...
        boolean pendingRestart = spec.activationPolicy() == ConfigActivationPolicy.RESTART_REQUIRED && storedValue != null && persistedRevision != snapshot.revision();

        return new ConfigManagementView(spec.key().value(), spec.module(), spec.group(), spec.title(),
                spec.description(), spec.schemaVersion(), persistedRevision, snapshot.revision(), configured,
                spec.activationPolicy(), spec.editPolicy(), snapshot.source(),
                status(spec, snapshot, configured, pendingRestart), pendingRestart,
                ConfigValueTreeSupport.redact(effectiveValue, spec.fields()),
                ConfigValueTreeSupport.redact(persisted.value(), spec.fields()),
                ConfigValueTreeSupport.redact(defaultValue, spec.fields()), spec.fields(),
                ConfigValueTreeSupport.sensitivePresence(persisted.presenceSource(), spec.fields()), spec.editorId(),
                snapshot.loadWarning());
    }

    private <T> PersistedTree persistedTree(ConfigSpec<T> spec, ConfigValue storedValue, JsonNode defaultValue) {
    ...
    private ConfigManagementStatus status(ConfigSpec<?> spec, ConfigSnapshot<?> snapshot, boolean configured,
