...
public class ConfigValueRepositoryJpaAdapter implements ConfigValueRepository {
  ...
  public List<ConfigValue> findAll() {
    return jpaRepository.findAllByOrderByConfigKeyAsc().stream()
        .map(ConfigValueEntity::toDomain)
        .toList();
  }
  ...
  public boolean update(ConfigValue configValue, long expectedRevision) {
    if (expectedRevision < 1 || configValue.revision() != Math.addExact(expectedRevision, 1)) {
      throw new IllegalArgumentException(
          "configValue revision must immediately follow expectedRevision");
    }
    int updated =
        jpaRepository.updateByExpectedRevision(
            configValue.configKey(),
            configValue.content(),
            configValue.schemaVersion(),
            configValue.revision(),
            configValue.configured(),
            configValue.updatedBy(),
            configValue.updatedAt(),
            configValue.updateReason(),
            expectedRevision);
    return updated == 1;
  }
}
