 ...
 * 用户包功能授权。
 *
 * <p>表示用户应用包在部分功能访问模式下可使用的应用内功能。</p>
 *
 * @author Corwin
 */
@Getter
@Entity
@Table(name = "sys_user_package_feature_access", indexes = {
        @Index(name = "idx_sys_user_package_feature_access_unique", columnList = "package_id,feature_id",
                unique = true),
        @Index(name = "idx_sys_user_package_feature_access_package_id", columnList = "package_id"),
        @Index(name = "idx_sys_user_package_feature_access_application_id", columnList = "application_id"),
...
public class UserPackageFeatureAccess {
     ...
     */
    public UserPackageFeatureAccess(Long packageId, Long applicationId, Long featureId, Boolean systemBuiltIn,
            Long operator) {
