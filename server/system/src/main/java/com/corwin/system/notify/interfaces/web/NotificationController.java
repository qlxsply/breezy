 ...
 *
 * <p>Exposes endpoints for querying, pulling, and marking notifications as read.
 *
...
public class NotificationController {
  ...
  public ApiResponse<PageResult<NotificationRes>> page(@RequestBody NotificationPageReq req) {
    ...
    PageData<NotificationView> pageData =
        notificationAppService.page(userId, userType, status, spec);
   ...
   * 拉取指定时间点之后的未读通知
   *
   * <p>按 createdAt 升序返回，用于增量拉取。
   */
  @GetMapping("/unread/pull")
  @Authenticated
  public ApiResponse<NotificationPullRes> pullUnread(
      @RequestParam(required = false, defaultValue = "0") long after,
      @RequestParam(defaultValue = "50") int limit) {
    Long userId = CtxUtil.getPrincipal().userId();
    UserType userType = CtxUtil.getPrincipal().userType();
    return ApiResponse.ok(
        toPullRes(notificationAppService.pullUnread(userId, userType, after, limit)));
  }
  ...
  private static NotificationPullRes toPullRes(NotificationPullView view) {
    return new NotificationPullRes(
        view.items().stream().map(NotificationController::toRes).toList(), view.lastPullAt());
  }
}
