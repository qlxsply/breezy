...
public class PushSubscriptionAppService {
  ...
  public void saveSubscription(
      Long userId, UserType userType, SavePushSubscriptionCommand command) {
    ...
    log.info(
        "Saving push subscription: userType={}, userId={}, deviceId={}",
        userType,
        userId,
        command.deviceId());

    userPushSubscriptionRepository
        .findByUserIdAndUserTypeAndDeviceId(userId, userType, command.deviceId())
        .ifPresentOrElse(
            existing -> {
              existing.refresh(command.endpoint(), command.p256dh(), command.auth());
              userPushSubscriptionRepository.save(existing);
              log.info(
                  "Push subscription refreshed: userType={}, userId={}, deviceId={}",
                  userType,
                  userId,
                  command.deviceId());
            },
            () -> {
              ...
              log.info(
                  "Push subscription created: userType={}, userId={}, deviceId={}",
                  userType,
                  userId,
                  command.deviceId());
            });
  ...
  public void removeSubscription(Long userId, UserType userType, String deviceId) {
    ...
    if (deviceId == null || deviceId.isBlank()) {
      log.warn(
          "Skip remove push subscription because deviceId is blank, userType={}, userId={}",
          userType,
          userId);
      return;
    }
    ...
    log.info(
        "Removing push subscription: userType={}, userId={}, deviceId={}",
        userType,
        userId,
        normalizedDeviceId);

    userPushSubscriptionRepository
        .findByUserIdAndUserTypeAndDeviceId(userId, userType, normalizedDeviceId)
        .ifPresent(
            subscription -> {
              subscription.deactivate("用户主动取消订阅");
              userPushSubscriptionRepository.save(subscription);
              log.info(
                  "Push subscription deactivated: userType={}, userId={}, deviceId={}",
                  userType,
                  userId,
                  normalizedDeviceId);
            });
