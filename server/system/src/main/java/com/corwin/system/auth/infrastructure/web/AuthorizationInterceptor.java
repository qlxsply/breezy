...
/**
 * Handler interceptor that enforces {@link PermitAll}, {@link Authorize}, and {@link Authenticated}
 * annotations on controller methods and classes.
 *
  ...
  public boolean preHandle(
    ...
    if (methodAuthorize != null) {
      authorizationService.checkAuthorized(
          methodAuthorize.userType(),
          methodAuthorize.permissions(),
    ...
    if (methodAuthenticated != null) {
      authorizationService.checkAuthenticated(
          methodAuthenticated.userType(),
          methodAuthenticated.allowExpiredCredentials(),
          methodAuthenticated.optional());
      return true;
    }
    ...
    if (typeAuthorize != null) {
      authorizationService.checkAuthorized(
          typeAuthorize.userType(), typeAuthorize.permissions(), typeAuthorize.anyPermission());
      return true;
    }

    Authenticated typeAuthenticated =
        handlerMethod.getBeanType().getAnnotation(Authenticated.class);
    if (typeAuthenticated != null) {
      authorizationService.checkAuthenticated(
          typeAuthenticated.userType(),
          typeAuthenticated.allowExpiredCredentials(),
          typeAuthenticated.optional());
      return true;
    }
