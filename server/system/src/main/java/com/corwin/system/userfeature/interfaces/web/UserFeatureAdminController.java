 ...
 *
 * <p>Provides endpoints for managing product applications, user application packages, and per-user
 * feature access overrides in the admin console.
 *
...
public class UserFeatureAdminController {
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"ufa.view", "ufp.view", "ufp.edit"},
      anyPermission = true)
  public ApiResponse<List<UserFeatureApplicationRes>> applicationCatalog() {
    ...
    return ApiResponse.ok(
        applications.stream().map(UserFeatureAdminController::toApplicationRes).toList());
  }
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"ufa.edit"})
  @Audit(
      resource = AuditResource.NORMAL_FEATURE,
      action = AuditAction.UPDATE,
      level = AuditLevel.HIGH)
  public ApiResponse<Boolean> updateApplicationStatus(
      @PathVariable Long id, @RequestBody UpdateUserFeatureStatusReq req) {
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"ufp.view"})
  public ApiResponse<PageResult<UserFeaturePackageRes>> pagePackages(
      @RequestBody UserFeaturePackagePageReq req) {
    var page =
        userFeatureAdminService.pagePackages(
            req.keyword(), req.enabled(), PageSpecFactory.of(req.page(), req.sort()));
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"ufp.edit"})
  @Audit(
      resource = AuditResource.NORMAL_FEATURE,
      action = AuditAction.CREATE,
      description = "创建用户应用包",
      level = AuditLevel.HIGH)
  public ApiResponse<UserFeaturePackageRes> createPackage(
      @RequestBody SaveUserFeaturePackageReq req) {
    return ApiResponse.ok(
        toPackageRes(userFeatureAdminService.createPackage(toPackageCommand(req))));
  }
  ...
  @Audit(
      ...
      description = "更新用户应用包",
      level = AuditLevel.HIGH)
  public ApiResponse<UserFeaturePackageRes> updatePackage(
      @PathVariable Long id, @RequestBody SaveUserFeaturePackageReq req) {
    return ApiResponse.ok(
        toPackageRes(userFeatureAdminService.updatePackage(id, toPackageCommand(req))));
  }
  ...
  @Audit(
      resource = AuditResource.NORMAL_FEATURE,
      action = AuditAction.UPDATE,
      description = "更新用户应用包状态",
      level = AuditLevel.HIGH)
  public ApiResponse<Boolean> updatePackageStatus(
      @PathVariable Long id, @RequestBody UpdateUserFeatureStatusReq req) {
   ...
   *
   * <p>Also removes all associated application/feature access records and package member mappings.
   *
   * @param id the package ID
   * @return true on success
   */
   ...
   *
   * <p>Returns assigned packages, application access state with inherited/effective visibility, and
   ...
   *
   * <p>Updates package assignments, application-level overrides, and feature-level overrides.
   *
   * @param userId the user ID
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"ufu.edit"})
  @Audit(
      resource = AuditResource.NORMAL_FEATURE,
      action = AuditAction.UPDATE_USER,
      description = "维护用户应用功能配置",
      level = AuditLevel.HIGH)
  public ApiResponse<Boolean> saveUserManagement(
      @PathVariable Long userId, @RequestBody SaveUserFeatureUserManagementReq req) {
    return ApiResponse.ok(
        userFeatureAdminService.saveUserManagement(userId, toUserManagementCommand(req)));
  }

  private static SaveUserFeaturePackageCommand toPackageCommand(SaveUserFeaturePackageReq req) {
    var accesses =
        req.applicationAccesses() == null
            ? List.<SaveUserFeaturePackageCommand.ApplicationAccessCommand>of()
            : req.applicationAccesses().stream()
                .map(
                    item ->
                        new SaveUserFeaturePackageCommand.ApplicationAccessCommand(
                            item.applicationId(), item.featureAccessScope(), item.featureIds()))
                .toList();
    return new SaveUserFeaturePackageCommand(
        req.code(),
        req.name(),
        req.packageType(),
        req.description(),
  ...
  private static SaveUserFeatureUserManagementCommand toUserManagementCommand(
      SaveUserFeatureUserManagementReq req) {
    var appOverrides =
        req.applicationOverrides() == null
            ? List.<ApplicationOverrideCommand>of()
            : req.applicationOverrides().stream()
                .map(
                    item ->
                        new ApplicationOverrideCommand(
                            item.applicationId(), item.overrideType(), item.featureAccessScope()))
                .toList();
    var featureOverrides =
        req.featureOverrides() == null
            ? List.<FeatureOverrideCommand>of()
            : req.featureOverrides().stream()
                .map(
                    item ->
                        new FeatureOverrideCommand(
                            item.applicationId(), item.featureId(), item.overrideType()))
                .toList();
    return new SaveUserFeatureUserManagementCommand(
        req.packageIds(), appOverrides, featureOverrides);
  }

  private static UserFeatureApplicationRes toApplicationRes(UserFeatureApplicationView view) {
    return new UserFeatureApplicationRes(
  ...
  private static UserFeatureItemRes toItemRes(UserFeatureItemView view) {
    return new UserFeatureItemRes(
        String.valueOf(view.id()),
        String.valueOf(view.applicationId()),
        view.code(),
        view.name(),
        view.description(),
        view.enabled(),
  ...
  private static UserFeaturePackageRes toPackageRes(UserFeaturePackageView view) {
    return new UserFeaturePackageRes(
        String.valueOf(view.id()),
        view.code(),
        view.name(),
        view.packageType(),
        ...
        view.enabled(),
        view.defaultPackage(),
        view.applicationAccesses().stream()
            .map(
                item ->
                    new UserFeaturePackageApplicationAccessRes(
                        item.applicationId(),
                        item.applicationCode(),
                        item.applicationName(),
                        item.featureAccessScope(),
                        item.featureIds(),
                        item.features().stream()
                            .map(UserFeatureAdminController::toItemRes)
                            .toList()))
            .toList());
  }

  private static UserFeatureUserManagementRes toUserManagementRes(
      UserFeatureUserManagementView view) {
    return new UserFeatureUserManagementRes(
        view.userId(),
        view.account(),
        view.packageIds(),
        view.packages().stream()
            .map(
                item ->
                    new UserFeaturePackageOptionRes(
                        item.id(),
                        item.code(),
                        item.name(),
                        item.packageType(),
                        ...
                        item.defaultPackage()))
            .toList(),
        view.applications().stream()
            .map(
                item ->
                    new UserFeatureUserApplicationRes(
                        item.id(),
                        item.code(),
                        item.name(),
                        item.description(),
                        ...
                        item.componentPath(),
                        item.enabled(),
                        item.inheritedVisible(),
                        item.effectiveVisible(),
                        item.packageAccessScope(),
                        item.overrideType(),
                        item.overrideAccessScope(),
                        item.features().stream()
                            .map(
                                feature ->
                                    new UserFeatureUserFeatureRes(
                                        feature.id(),
                                        feature.applicationId(),
                                        feature.applicationCode(),
                                        feature.code(),
                                        feature.name(),
                                        feature.description(),
                                        feature.enabled(),
                                        feature.permissionCodes(),
                                        feature.inheritedEnabled(),
                                        feature.effectiveEnabled(),
                                        feature.overrideType()))
                            .toList()))
            .toList());
  }
}
