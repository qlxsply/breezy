...
public class MethodStatAggregate {
    ...
    public synchronized MethodStatAggregateSnapshot snapshot(long nowMillis) {
        ...
        return new MethodStatAggregateSnapshot(key, totalCalls, totalSuccess, totalFailure, secondWindow.total(),
                minuteWindow.total(), hourWindow.total(), secondWindow.success(), secondWindow.failure(),
                minuteWindow.success(), minuteWindow.failure(), hourWindow.success(), hourWindow.failure(),
    ...
    private MethodStatDurationMetrics calculateDurationMetrics() {
        ...
        return new MethodStatDurationMetrics(durationSampleCount, min, max, sum / durationSampleCount,
                percentile(samples, 50), percentile(samples, 90), percentile(samples, 95), percentile(samples, 99));
    }
    ...
    private BucketCount sumWindow(WindowBucket[] buckets, long minSlotInclusive, long maxSlotInclusive) {
        long total = 0L;
        long success = 0L;
        long failure = 0L;
