 ...
 *
 * <p>Provides endpoints for listing, searching, enabling/disabling APIs, and updating sort
 * configurations. All endpoints require ADMIN authentication.
 *
...
public class ApiAdminController {
  ...
  public ApiResponse<PageResult<ApiRes>> page(@RequestBody ApiPageReq req) {
    ...
    ApiPageQuery query =
        new ApiPageQuery(
            ...
            req.handlerClass(),
            req.handlerMethod(),
            resolvePermissionDeclared(req.permissionDeclared()),
            resolveAccessType(req.accessType()),
            resolveUserType(req.userType()),
            resolveAuditDeclared(req.auditDeclared()),
  ...
  public ApiResponse<ApiRes> updateSortOptions(
      @PathVariable Long id, @RequestBody UpdateApiSortOptionsReq req) {
  ...
  private ApiRes toDto(Api api) {
    return new ApiRes(
        ...
        api.getPathPattern(),
        api.getHandlerClass(),
        api.getHandlerMethod(),
        Boolean.TRUE.equals(api.getPermissionDeclared()),
        api.getAccessType(),
        api.getUserType(),
        Boolean.TRUE.equals(api.getAuditDeclared()),
        api.getAuditResource(),
        api.getAuditAction(),
        api.getAuditDescription(),
        api.getSortOptionsJson(),
