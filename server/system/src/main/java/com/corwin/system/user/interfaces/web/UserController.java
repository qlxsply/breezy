...
/**
 * REST controller for end-user (non-admin) self-service operations including registration, profile
...
public class UserController {
   ...
   *
   * @param req the registration request containing username, nickname, and password
  ...
  public ApiResponse<UserProfileRes> register(@RequestBody RegisterUserReq req) {
    RegisterUserCommand cmd =
        new RegisterUserCommand(req.username(), req.nickname(), req.password());
  ...
  public ApiResponse<UserProfileRes> updateMyProfile(@RequestBody UpdateMyProfileReq req) {
    return ApiResponse.ok(
        toRes(userService.updateMyProfile(new UpdateMyProfileCommand(req.nickname()))));
  }

  /**
   * Changes the password for the currently authenticated end-user.
   *
   * @param req the change password request containing old and new passwords
