 ...
 *
 * <p>Handles CRUD operations, sorting, status toggling, and draft-based batch item synchronization.
 *
...
public class DictAdminService {
  ...
  public List<DictTypeView> listTypes(String keyword) {
    ...
    return types.stream()
        .filter(item -> contains(item.getCode(), kw) || contains(item.getName(), kw))
        .map(this::toTypeView)
        .toList();
  }
  ...
  public PageData<DictTypeView> pageTypes(String code, String name, PageSpec spec) {
    ...
    return new PageData<>(
        ...
        page.totalPages(),
        page.totalElements(),
        page.elements().stream().map(this::toTypeView).toList());
  }
  ...
  public DictTypeView createType(CreateDictTypeCommand cmd) {
    ...
    DictType entity =
        new DictType(
            ...
            trimToNull(cmd.description(), TYPE_DESCRIPTION_MAX),
            trimToNull(cmd.enumClass(), TYPE_DESCRIPTION_MAX),
            defaultValueType(cmd.valueType()),
            defaultStructureType(cmd.structureType()),
            DictSourceType.CUSTOM,
            cmd.enabled(),
            operator);
  ...
  public DictItemView createItem(String typeId, CreateDictItemCommand cmd) {
    ...
    BizAssert.state(
        !dictItemRepository.existsByDictTypeIdAndItemCode(type.getId(), itemCode),
        BaseError.CONFLICT);
    BizAssert.state(
        !dictItemRepository.existsByDictTypeIdAndItemValue(type.getId(), itemValue),
        BaseError.CONFLICT);
    DictItem item =
        new DictItem(
            newId(),
            ...
            parentItemId,
            itemCode,
            normalizeItemLabel(cmd.itemLabel()),
            itemValue,
            normalizeSortNo(cmd.sortNo()),
            enabled,
            cmd.defaultItem(),
            trimToNull(cmd.tagColor(), ITEM_TAG_COLOR_MAX),
            trimToNull(cmd.tagType(), ITEM_TAG_TYPE_MAX),
            trimToNull(cmd.extraJson(), ITEM_EXTRA_JSON_MAX),
            trimToNull(cmd.description(), ITEM_DESCRIPTION_MAX),
            operator());
  ...
  public DictItemView updateItem(String itemId, UpdateDictItemCommand cmd) {
    ...
    boolean enabled = normalizeItemEnabled(cmd.enabled(), cmd.defaultItem());
    DictItem existingValueItem =
        dictItemRepository.findByDictTypeIdAndItemValue(type.getId(), nextValue).orElse(null);
  ...
  public boolean deleteItem(String itemId) {
    DictItem item = getItemEntity(itemId);
    BizAssert.state(
        dictItemRepository.findByParentItemId(item.getId()).isEmpty(), BaseError.FORBIDDEN);
  ...
  public boolean sortItems(String typeId, List<String> itemIds) {
    ...
    List<DictItem> items =
        dictItemRepository.findByDictTypeIdOrderBySortNoAscItemLabelAsc(type.getId());
  ...
  /**
   * Synchronizes the draft-based item list of a dictionary type: creates new items, updates
   * existing ones, and deletes removed ones.
   */
  private void syncTypeItems(
      DictType type, List<SaveDictTypeItemCommand> itemCommands, String operator) {
    List<SaveDictTypeItemCommand> safeCommands = itemCommands == null ? List.of() : itemCommands;
    List<DictItem> existingItems =
        dictItemRepository.findByDictTypeIdOrderBySortNoAscItemLabelAsc(type.getId());
    ...
    Map<String, DraftItemPayload> payloadByClientKey =
        normalizeDraftItems(type, safeCommands, existingById);
  ...
  private void normalizeDefaultItem(String dictTypeId, String keepId, String operator) {
    for (DictItem existing :
        dictItemRepository.findByDictTypeIdOrderBySortNoAscItemLabelAsc(dictTypeId)) {
  ...
  private Map<String, DraftItemPayload> normalizeDraftItems(
      DictType type, List<SaveDictTypeItemCommand> items, Map<String, DictItem> existingById) {
    ...
    for (SaveDictTypeItemCommand item : items) {
      ...
      payloadByClientKey.put(
          clientKey,
          new DraftItemPayload(
              id,
              clientKey,
              trimToNull(item.parentClientKey()),
              itemCode,
              itemLabel,
              itemValue,
              normalizeSubmittedSortNo(item.sortNo(), index),
              enabled,
              defaultItem,
              trimToNull(item.tagColor(), ITEM_TAG_COLOR_MAX),
              trimToNull(item.tagType(), ITEM_TAG_TYPE_MAX),
  ...
  /**
   * Validates parent-child relationships among draft items to prevent cycles and invalid
   * references.
   */
  private void validateDraftItemRelations(
      DictType type, Map<String, DraftItemPayload> payloadByClientKey) {
  ...
  private String resolveParentItemId(
      String parentClientKey, Map<String, DictItem> entityByClientKey, String selfId) {
