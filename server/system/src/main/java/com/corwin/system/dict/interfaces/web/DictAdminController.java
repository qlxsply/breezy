 ...
 *
 * <p>Provides admin CRUD endpoints for managing dictionary types and their items.
 *
...
public class DictAdminController {
  ...
  public ApiResponse<PageResult<DictTypeRes>> list(@RequestBody DictTypePageReq req) {
    return ApiResponse.ok(
        PageResult.of(
            dictAdminService.pageTypes(
                req.code(), req.name(), PageSpecFactory.of(req.page(), req.sort())),
            this::toTypeRes));
  }
  ...
  public ApiResponse<DictTypeRes> create(@RequestBody CreateDictTypeReq req) {
    DictTypeView view =
        dictAdminService.createType(
            new CreateDictTypeCommand(
                req.code(),
                req.name(),
                req.description(),
                req.enumClass(),
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"dict.edit"})
  public ApiResponse<DictTypeRes> update(
      @PathVariable String id, @RequestBody UpdateDictTypeReq req) {
    DictTypeView view =
        dictAdminService.updateType(
            id,
            new UpdateDictTypeCommand(
                req.name(),
                req.description(),
                req.enumClass(),
                req.valueType(),
  ...
  public ApiResponse<Boolean> updateStatus(
      @PathVariable String id, @RequestBody UpdateDictStatusReq req) {
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"dict.edit"})
  public ApiResponse<DictItemRes> createItem(
      @PathVariable String id, @RequestBody CreateDictItemReq req) {
    DictItemView view =
        dictAdminService.createItem(
            id,
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"dict.edit"})
  public ApiResponse<DictItemRes> updateItem(
      @PathVariable String itemId, @RequestBody UpdateDictItemReq req) {
    DictItemView view =
        dictAdminService.updateItem(
            itemId,
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"dict.edit"})
  public ApiResponse<Boolean> updateItemStatus(
      @PathVariable String itemId, @RequestBody UpdateDictStatusReq req) {
  ...
  @Authorize(
      userType = UserType.ADMIN,
      permissions = {"dict.edit"})
  public ApiResponse<Boolean> sortItems(
      @PathVariable String id, @RequestBody UpdateDictItemSortReq req) {
    return ApiResponse.ok(dictAdminService.sortItems(id, req.itemIds()));
  }

  private DictTypeRes toTypeRes(DictTypeView view) {
    return new DictTypeRes(
        view.id(),
        view.code(),
        view.name(),
        view.description(),
