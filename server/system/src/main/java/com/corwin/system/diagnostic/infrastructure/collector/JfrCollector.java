...
/**
 * Collector that manages a JFR (Java Flight Recorder) streaming session to monitor GC pauses,
 * exceptions, thread park, and monitor contention events.
 *
...
public class JfrCollector {
  ...
  /**
   * Starts a JFR recording stream. Enables GC events by default; additionally enables exception,
   * thread park, and monitor events when deep mode is configured.
   *
  ...
  /**
   * Checks whether JFR is available in the current JVM. Always returns true as JFR is built into
   * JDK 21+.
   *
   * @return true
   */
  ...
  public JfrSnapshot snapshot() {
    return new JfrSnapshot(
        running.get(),
        gcEventCount.sum(),
        gcPauseTimeMs.sum(),
        exceptionEventCount.sum(),
        threadParkEventCount.sum(),
        monitorBlockedEventCount.sum());
  }
  ...
  private void handleExceptionEvent(RecordedEvent event) {
    ...
    eventRepository.append(
        new DiagnosticEvent(
            UUID.randomUUID().toString(),
            DiagnosticEventType.JFR_EXCEPTION,
            Instant.now(),
            "异常热点",
            throwableClass,
            Map.of("message", stringValue(event, "message"))));
  }
  ...
  private void handleThreadParkEvent(RecordedEvent event) {
    ...
    eventRepository.append(
        new DiagnosticEvent(
            UUID.randomUUID().toString(),
            DiagnosticEventType.JFR_THREAD_PARK,
            Instant.now(),
            "线程等待",
            event.getEventType().getName(),
            Map.of(
                "durationMs", safeDurationMs(event), "class", stringValue(event, "parkedClass"))));
  }
  ...
  private void handleMonitorBlockedEvent(RecordedEvent event) {
    ...
    eventRepository.append(
        new DiagnosticEvent(
            UUID.randomUUID().toString(),
            DiagnosticEventType.JFR_MONITOR_BLOCKED,
            Instant.now(),
            "线程阻塞",
            event.getEventType().getName(),
            Map.of(
                "durationMs",
                safeDurationMs(event),
                "monitorClass",
                stringValue(event, "monitorClass"))));
  }
