...
public class DefaultRunAsService implements RunAsService {
    ...
    public void runAsSystem(Runnable action) {
        callAs(systemPrincipal(DefaultUser.SYSTEM), () -> {
            action.run();
            return null;
        });
    }
    ...
    public void runAsScheduler(Runnable action) {
        callAs(systemPrincipal(DefaultUser.SCHEDULER), () -> {
            action.run();
            return null;
        });
    }
    ...
    public void runAsEvent(Runnable action) {
        callAs(systemPrincipal(DefaultUser.EVENT), () -> {
            action.run();
            return null;
        });
    }
    ...
    private AuthPrincipal systemPrincipal(DefaultUser defaultUser) {
        return new AuthPrincipal(defaultUser.id(), defaultUser.account(), UserType.SYSTEM,
                DefaultUser.isAdmin(defaultUser.id()), Set.of());
    }
