...
/**
 * SSE 会话管理器 支持单例管理、心跳、以及基于内存缓冲区的 Last-Event-ID 补发。
 *
...
public class SseSessionManager {
  ...
  public void register(Long userId, UserType userType, SseEmitter emitter) {
    ...
    emitter.onError(
        (e) -> {
          log.debug(
              "SSE emitter error for userType={}, userId={}: {}", userType, userId, e.getMessage());
          removeIfSame(sessionKey, current);
        });
  }

  /** 向指定用户发送消息。 消息会自动加入内存缓冲区以支持断线补发。 */
   ...
   */
  public boolean send(
      Long userId, UserType userType, String eventName, Object data, String eventId) {
    String id = eventId;
  ...
  /**
   * Scheduled heartbeat that sends ping events to all active SSE sessions. Dead sessions are
  ...
  private record UserSessionKey(Long userId, UserType userType) {}

  private record UserSession(String sessionId, SseEmitter emitter) {}

  public record SseMessage(String id, String name, Object data) {}
}
