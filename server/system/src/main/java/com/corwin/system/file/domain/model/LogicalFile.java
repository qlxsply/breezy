 ...
 *
 * <p>Each logical file has an owner (user or application), a parent folder, and optionally
 * references a physical file on disk (for file nodes). Folder nodes have no physical file
 * reference.
 *
...
@Entity
@Table(
    name = "sys_storage_file",
    uniqueConstraints = {
      @UniqueConstraint(
          columnNames = {"owner_type", "owner_id", "parent_id", "node_type", "file_name"})
    })
public class LogicalFile {
   ...
   */
  public static LogicalFile file(
      OwnerType ownerType,
      String ownerId,
      String parentId,
      String fileName,
   ...
   */
  public static LogicalFile folder(
      OwnerType ownerType, String ownerId, String parentId, String fileName) {
    return new LogicalFile(
        ownerType, ownerId, parentId, LogicalNodeType.FOLDER, fileName, null, null);
  }
  ...
  /**
   * Replaces the physical file reference of this logical file node. Only valid for file-type nodes;
   * folder nodes will throw an exception.
   */
