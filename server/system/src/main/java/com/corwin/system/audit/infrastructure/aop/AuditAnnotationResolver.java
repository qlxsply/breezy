...
/**
 * Resolves audit metadata from {@link Audit} annotations at runtime. Caches resolved {@link
 * AuditOperation} instances keyed by method and implementation class to avoid repeated reflection
 * lookups.
 *
...
public class AuditAnnotationResolver {
  ...
  /**
   * Resolves the audit operation metadata for a given method and target class. Checks for {@link
   * Audit} and {@code ApiMeta} annotations, caches the result, and returns a resolved {@link
   * AuditOperation} or {@code null} if no annotation is present.
   *
  ...
  private Optional<AuditOperation> resolveOperation(CacheKey cacheKey) {
    ...
    Audit annotation =
        findAuditAnnotation(implementationMethod, declaredMethod, implementationClass);
    if (annotation == null) {
    ...
    return Optional.of(
        new AuditOperation(
            ...
            annotation.recordResponse(),
            applicationCode));
  }

  private Audit findAuditAnnotation(
      Method implementationMethod, Method declaredMethod, Class<?> implementationClass) {
    Audit annotation =
        AnnotatedElementUtils.findMergedAnnotation(implementationMethod, Audit.class);
    if (annotation != null) {
      return annotation;
    }
    ...
    return AnnotatedElementUtils.findMergedAnnotation(
        declaredMethod.getDeclaringClass(), Audit.class);
  }

  private ApiMeta findApiMeta(Class<?> implementationClass, Method declaredMethod) {
    ApiMeta apiMeta =
        AnnotatedElementUtils.findMergedAnnotation(implementationClass, ApiMeta.class);
    if (apiMeta != null) {
      return apiMeta;
    }
    return AnnotatedElementUtils.findMergedAnnotation(
        declaredMethod.getDeclaringClass(), ApiMeta.class);
  }
  ...
  private String buildDescription(
      String rawDescription, AuditResource resource, AuditAction action) {
  ...
  private record CacheKey(Method declaredMethod, Class<?> implementationClass) {}
}
