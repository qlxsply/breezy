...
/**
 * Repository interface for {@link LogicalFile} entity. Provides data access for logical file and
 * folder nodes with various query methods based on ownership, parent, node type, and file name.
 *
...
public interface LogicalFileRepository extends JpaRepository<LogicalFile, String> {
  ...
  List<LogicalFile> findByOwnerTypeAndOwnerIdAndParentIdAndNodeType(
      OwnerType ownerType, String ownerId, String parentId, LogicalNodeType nodeType);

  /** Finds all nodes (files and folders) under a given owner and parent. */
  List<LogicalFile> findByOwnerTypeAndOwnerIdAndParentId(
      OwnerType ownerType, String ownerId, String parentId);

  /** Finds all nodes (across all owners) under a given parent ID. */
  List<LogicalFile> findByParentId(String parentId);

  /**
   * Finds a specific node by owner, parent, node type, and exact file name. Used for conflict
   * detection (e.g. duplicate name check).
   */
  Optional<LogicalFile> findByOwnerTypeAndOwnerIdAndParentIdAndNodeTypeAndFileName(
      OwnerType ownerType,
      String ownerId,
      String parentId,
      LogicalNodeType nodeType,
      String fileName);
