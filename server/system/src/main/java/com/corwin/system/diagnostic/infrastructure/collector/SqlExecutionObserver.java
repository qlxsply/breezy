...
/**
 * Observer that tracks SQL execution metrics including per-statement aggregates, slow SQL and error
 * counts, and emits corresponding diagnostic events.
 *
...
public class SqlExecutionObserver {
  ...
  /**
   * Records a single SQL execution and updates all relevant counters and aggregates. Emits slow SQL
   * and SQL error events when thresholds or errors are encountered.
   *
  ...
  public void record(
    ...
    SqlAggregate aggregate =
        sqlStats.computeIfAbsent(
            normalizedDataSourceName + "|" + normalizedSql,
  ...
  public SqlSnapshot snapshot() {
    ...
    return new SqlSnapshot(
        total,
        total <= 0L ? 0D : (double) duration / total,
        slowSqlCount.sum(),
        errorCount.sum(),
        sqlStats.values().stream()
            .sorted(
                Comparator.comparingLong(SqlAggregate::totalExecutionsValue)
                    .reversed()
  ...
  private static final class SqlAggregate {
    ...
    private SqlStatementStatSnapshot toSnapshot() {
      ...
      long duration = totalDurationMs.sum();
      return new SqlStatementStatSnapshot(
          dataSourceName,
          sql,
          executions,
          duration,
          executions <= 0L ? 0L : duration / executions,
          slowExecutions.sum(),
          errorExecutions.sum(),
          maxDurationMs.get());
    }
