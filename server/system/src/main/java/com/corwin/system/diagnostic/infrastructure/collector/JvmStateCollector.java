...
/**
 * Collector that captures JVM runtime state including memory, class loading, GC activity, and CPU
 * usage.
 *
...
public class JvmStateCollector {
  ...
  public JvmSnapshot collect() {
    ...
    List<MemoryPoolSnapshot> pools =
        ManagementFactory.getMemoryPoolMXBeans().stream().map(this::toMemoryPoolSnapshot).toList();
    ...
    return new JvmSnapshot(
        runtime.getStartTime() <= 0L
            ? null
            : java.time.Instant.ofEpochMilli(runtime.getStartTime()),
        runtime.getUptime(),
  ...
  private MemoryPoolSnapshot toMemoryPoolSnapshot(MemoryPoolMXBean bean) {
    ...
    return new MemoryPoolSnapshot(
        bean.getName(), usage.getUsed(), usage.getCommitted(), usage.getMax());
  }
