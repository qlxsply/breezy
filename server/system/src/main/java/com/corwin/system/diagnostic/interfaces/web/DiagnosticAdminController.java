...
/**
 * REST controller providing administrative API endpoints for the diagnostic system. Supports
 * starting/stopping diagnostic sessions, updating configuration, querying status, retrieving
 * snapshots, filtering events, and inspecting runtime capabilities.
 *
...
public class DiagnosticAdminController {
  ...
  public DiagnosticAdminController(
      DiagnosticCommandAppService commandAppService, DiagnosticQueryAppService queryAppService) {
  ...
  public ApiResponse<DiagnosticSessionView> start(@RequestBody StartDiagnosticReq req) {
    return ApiResponse.ok(
        commandAppService.start(
            new StartDiagnosticCommand(
                req.intervalMs(),
                req.historyCapacity(),
                req.eventCapacity(),
                req.items(),
                req.deepMode(),
                req.slowRequestThresholdMs(),
                req.slowSqlThresholdMs(),
                req.ttlSeconds())));
  }
   ...
   */
  public ApiResponse<DiagnosticSessionView> updateConfig(
      @RequestBody UpdateDiagnosticConfigReq req) {
    return ApiResponse.ok(
        commandAppService.updateConfig(
            new UpdateDiagnosticConfigCommand(
                req.intervalMs(),
                req.historyCapacity(),
                req.eventCapacity(),
                req.items(),
