...
public class LocalCacheHashOps<T> extends AbstractLocalCacheOps implements CacheHashOps<T> {
    ...
    public Optional<T> get(String key, String field) {
        ...
        return store.read(dataType(), key, entry -> {
            Map<String, Object> values = asMap(entry);
            return Optional.ofNullable(values.get(field)).map(value -> TypeSafeValueCaster.cast(value, valueType));
        }, Optional::empty);
    }
    ...
    private CacheEntry upsert(CacheEntry entry, Map<String, Object> values, Instant now, Duration ttl) {
