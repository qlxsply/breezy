...
public final class ConfigSnapshotFactory {
        ...
        try {
            if (schemaVersion != spec.schemaVersion()) {
                throw new ConfigLoadException(
                        "Schema version mismatch: stored=" + schemaVersion + ", expected=" + spec.schemaVersion()
                );
            }
    ...
    private static <T> void validateRuntime(ConfigSpec<T> spec, T value) {
        ...
        if (violations != null && !violations.isEmpty()) {
            throw new ConfigValidationException("Config runtime validation failed: " + spec.key(), violations);
        }
    }
    ...
    private static String warning(RuntimeException ex) {
        ...
        String warning = ex.getClass().getSimpleName() + (message == null || message.isBlank() ? "" : ": " + message);
        return warning.length() <= WARNING_MAX_LENGTH ? warning : warning.substring(0, WARNING_MAX_LENGTH);
    }
