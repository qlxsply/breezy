...
public class GlobalExceptionAdvice {
  ...
  public ApiResponse<List<ConfigViolation>> handleConfigValidationException(
      ConfigValidationException ex) {
    log.warn(
        "ConfigValidationException msg={} violations={}", ex.getMessage(), ex.violations().size());
    return ApiResponse.fail(
        ex.violations(), BaseError.INVALID_PARAMETER.getCode(), ex.getMessage());
  }
  ...
  public ResponseEntity<ApiResponse<Object>> handleBizException(BizException ex) {
    ...
    HttpStatusCode status =
        errorHttpStatusResolvers.stream()
            .map(resolver -> resolver.resolve(ex.getErrorCode()))
