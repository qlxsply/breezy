 ...
 *
 * <ul>
 *   <li>Generate or propagate traceId from upstream headers
 *   <li>Generate spanId for the current node
 *   <li>Initialise the request context ({@link com.corwin.framework.web.ctx.Ctx})
 *   <li>Bind context data to MDC for log correlation
 *   <li>Write traceId back in the response header
 *   <li>Clean up the context after request completion
 * </ul>
 *
...
public class TraceFilter extends OncePerRequestFilter implements Ordered {
  ...
  private String resolveTraceId(String rawHeaderTraceId) {
    ...
    if (rawHeaderTraceId != null && !rawHeaderTraceId.isBlank()) {
      log.warn(
          "Ignore invalid {} header: {}", HttpHeaderNames.TRACE_ID, abbreviate(rawHeaderTraceId));
    }
  ...
  private String resolveParentSpanId(String rawHeaderSpanId) {
    ...
    if (rawHeaderSpanId != null && !rawHeaderSpanId.isBlank()) {
      log.warn(
          "Ignore invalid {} header: {}", HttpHeaderNames.SPAN_ID, abbreviate(rawHeaderSpanId));
    }
    return null;
  }
