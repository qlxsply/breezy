 ...
 */
@Slf4j
public final class ConfigBootstrapEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        ...
        long overrideCount = snapshots.values().stream().filter(snapshot -> snapshot.revision() > 0).count();
        long orphanCount = storedValues.keySet().stream().filter(key -> !specs.containsKey(key)).count();
        log.info("Unified config registry initialized: definitions={}, persisted={}, orphaned={}", snapshots.size(),
                overrideCount, orphanCount);
    }
    ...
    private ConfigBootstrapValueLoader discoverLoader(ClassLoader classLoader) {
        var providers = ServiceLoader.load(ConfigBootstrapValueLoader.class, classLoader).stream().toList();
        if (providers.size() != 1) {
    ...
    private Map<String, ConfigSnapshot<?>> createSnapshots(Map<String, ConfigSpec<?>> specs,
            Map<String, RawConfigValue> storedValues) {
    ...
    private <T> ConfigSnapshot<T> createSnapshot(ConfigSpec<T> spec, RawConfigValue storedValue) {
        ...
        return ConfigSnapshotFactory.createStored(spec, storedValue.content(), storedValue.schemaVersion(),
                storedValue.revision(), storedValue.configured());
    }
}
