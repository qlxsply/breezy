...
public class AsyncEventListenerBeanPostProcessor implements BeanPostProcessor {
    ...
    public AsyncEventListenerBeanPostProcessor(SubscriptionRegistry registry, ConsumerGroupResolver groupResolver,
        ...
        this.applicationName = StringUtils.hasText(configuredName) ? configuredName.trim() : "application";
    }
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) {
        ...
        ReflectionUtils.doWithMethods(targetClass, method -> processMethod(bean, targetClass, method),
    ...
    private void processMethod(Object bean, Class<?> targetClass, Method method) {
        ...
        AsyncEventListener annotation = AnnotatedElementUtils.findMergedAnnotation(bridgedMethod,
        ...
        String rawGroup = resolveRawGroup(annotation, targetClass, bridgedMethod, signature.eventClass().getName());
        ...
        SubscriptionDescriptor descriptor = new SubscriptionDescriptor(subscriptionId, subscriberName, rawGroup,
                consumerGroup, signature.eventClass(), signature.eventClass().getName(),
                createInvoker(bean, bridgedMethod, signature), SubscriptionMode.DEFAULT,
                new SubscriptionOptions(resolveSources(annotation), annotation.order(), annotation.allowSharedGroup()));
    ...
    private MethodSignature resolveSignature(Class<?> targetClass, Method method) {
        if (method.getParameterCount() != 1) {
            throw new IllegalStateException(
                    "AsyncEventListener method must have exactly one argument: " + describe(targetClass, method));
        }
        Class<?> parameterType = method.getParameterTypes()[0];
        if (AsyncEvent.class.isAssignableFrom(parameterType)) {
            @SuppressWarnings(
                    "unchecked") Class<? extends AsyncEvent> eventClass = (Class<? extends AsyncEvent>) parameterType;
        ...
        throw new IllegalStateException(
                "AsyncEventListener method argument must be AsyncEvent or AsyncEventEnvelope<T>: " + describe(
                        targetClass, method));
    }
     ...
     */
    private Class<? extends AsyncEvent> resolveEnvelopeEventClass(Class<?> targetClass, Method method) {
        Type genericType = method.getGenericParameterTypes()[0];
        if (!(genericType instanceof ParameterizedType parameterizedType)) {
            throw new IllegalStateException(
                    "AsyncEventEnvelope listener must declare payload generic type: " + describe(targetClass, method));
        }
        Type[] arguments = parameterizedType.getActualTypeArguments();
        if (arguments.length != 1) {
            throw new IllegalStateException(
                    "AsyncEventEnvelope listener generic argument count invalid: " + describe(targetClass, method));
        }
        Type argument = arguments[0];
        if (!(argument instanceof Class<?> eventClass) || !AsyncEvent.class.isAssignableFrom(eventClass)) {
            throw new IllegalStateException(
                    "AsyncEventEnvelope listener generic argument must implement AsyncEvent: " + describe(targetClass,
                            method));
        }
        @SuppressWarnings(
                "unchecked") Class<? extends AsyncEvent> typedEventClass = (Class<? extends AsyncEvent>) eventClass;
     ...
     */
    private void invoke(Object bean, Method method, MethodSignature signature, AsyncEventEnvelope<?> envelope) throws
            Exception {
        try {
            Object argument = signature.envelopeArgument() ? envelope : signature.eventClass().cast(envelope.payload());
            ...
            throw new IllegalStateException("Unexpected listener throwable: " + target.getClass().getName(), target);
        } catch (IllegalAccessException ex) {
            throw new IllegalStateException("Invoke async listener failed: " + describe(bean.getClass(), method), ex);
        }
    }
     ...
     */
    private String resolveRawGroup(AsyncEventListener annotation, Class<?> targetClass, Method method,
            String eventType) {
        if (StringUtils.hasText(annotation.group())) {
            return annotation.group().trim();
        }
        return applicationName + ":" + targetClass.getName() + "#" + method.getName() + "(" + eventType + ")";
    }

    /**
     * 解析订阅 ID，未显式配置时按稳定输入生成 name-based UUID。
     */
    private String resolveSubscriptionId(AsyncEventListener annotation, Class<?> targetClass, Method method,
            String rawGroup) {
    ...
    private List<String> resolveSources(AsyncEventListener annotation) {
        ...
        return Arrays.stream(annotation.sources()).filter(Objects::nonNull).map(String::trim)
    ...
    /**
     * 监听方法签名解析结果：
     * eventClass 表示监听的事件类型；envelopeArgument 表示方法是否以 Envelope 作为入参。
     */
    private record MethodSignature(
            Class<? extends AsyncEvent> eventClass,
            boolean envelopeArgument
    ) {
    }
}
