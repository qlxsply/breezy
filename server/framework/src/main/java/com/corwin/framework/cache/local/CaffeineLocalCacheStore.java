...
public class CaffeineLocalCacheStore implements LocalCacheStore, InitializingBean, DisposableBean {
    ...
    @Override
    public <R> R read(CacheDataType dataType, String key, Function<CacheEntry, R> reader, Supplier<R> absentSupplier) {
        try {
    ...
    @Override
    public <R> R write(CacheDataType dataType, String key,
    ...
    public boolean delete(CacheDataType dataType, String key) {
        return write(dataType, key, (entry, now) -> {
            if (entry == null) {
    ...
    public CacheTtlResult ttl(CacheDataType dataType, String key) {
        return read(dataType, key, entry -> {
    ...
    public void afterPropertiesSet() {
        ...
        cleanupExecutor.scheduleWithFixedDelay(cache::cleanUp, intervalMillis, intervalMillis, TimeUnit.MILLISECONDS);
    }
    ...
    private void validateDataType(String key, CacheDataType expectedType, CacheEntry entry) {
        if (entry.getDataType() != expectedType) {
            ...
            String msg = String.format("Cache key '%s' already exists as %s, expected %s", key, dataType, expectedType);
