...
/**
 * Default {@link ConsumeContextBinder} that saves the current thread context and MDC, restores the
 * snapshot context for consumption, and fully rolls back on scope close.
 *
...
public class DefaultConsumeContextBinder implements ConsumeContextBinder {
  ...
  private void bindCurrentContext(EventCtxSnapshot snapshot) {
    ...
    if (spanMode == ConsumeSpanMode.NEW_CHILD
        && snapshot.traceId() != null
        && !snapshot.traceId().isBlank()) {
