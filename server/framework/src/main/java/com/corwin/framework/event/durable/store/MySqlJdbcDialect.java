...
public class MySqlJdbcDialect implements JdbcDialect {
    ...
    public List<String> createTableSql(String eventTable, String deliveryTable) {
        String createEventSql = "create table if not exists " + eventTable + " ("
        ...
        String createDeliverySql = "create table if not exists " + deliveryTable + " ("
                + "id varchar(64) not null,"
    ...
    public String insertDeliverySql(String deliveryTable) {
        return "insert into " + deliveryTable + " ("
    ...
    public String claimDeliverySql(String deliveryTable) {
        return "update " + deliveryTable + " set "
    ...
    public String cleanupDeliverySql(String deliveryTable) {
        return "delete from " + deliveryTable + " where status in ('SUCCEEDED', 'CANCELLED') and completed_at is not null "
                + "and completed_at < ? limit ?";
    }

    @Override
    public String cleanupEventSql(String eventTable) {
        return "delete from " + eventTable + " where status = 'COMPLETED' and completed_at is not null and completed_at < ? "
                + "limit ?";
    }
}
