 ...
 */
public class LocalCacheSortedSetOps<T> extends AbstractLocalCacheOps implements CacheSortedSetOps<T> {

    private final Class<T> valueType;

    public LocalCacheSortedSetOps(LocalCacheStore store, CacheKeyValidator validator, Class<T> valueType) {
    ...
    public boolean add(String key, T member, double score) {
        ...
        return store.write(dataType(), key, (entry, now) -> {
            LocalSortedSetValue sortedSet = entry == null ? new LocalSortedSetValue() : asSortedSet(entry);
    ...
    public Optional<Long> reverseRank(String key, T member) {
        validator.validateKey(key);
        validator.validateValue(member);
        return store.read(dataType(), key, entry -> asSortedSet(entry).reverseRank(member), Optional::empty);
    }

    @Override
    public List<T> range(String key, long start, long end) {
        validator.validateKey(key);
        return store.read(dataType(), key, entry -> castList(asSortedSet(entry).range(start, end)), List::of);
    }

    @Override
    public List<T> reverseRange(String key, long start, long end) {
        validator.validateKey(key);
        return store.read(dataType(), key, entry -> castList(asSortedSet(entry).reverseRange(start, end)), List::of);
    }

    @Override
    public List<T> rangeByScore(String key, double minScore, double maxScore) {
        validator.validateKey(key);
        return store.read(dataType(), key, entry -> castList(asSortedSet(entry).rangeByScore(minScore, maxScore)),
                List::of);
    }
    ...
    public long countByScore(String key, double minScore, double maxScore) {
        validator.validateKey(key);
        return store.read(dataType(), key, entry -> asSortedSet(entry).countByScore(minScore, maxScore), () -> 0L);
    }
