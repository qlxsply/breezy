...
/**
 * Factory for creating count and page {@link org.apache.ibatis.mapping.BoundSql} instances by
 * cloning the original query's parameter mappings and additional parameters.
 *
...
public final class BoundSqlFactory {
  ...
  public static BoundSql forCount(Configuration configuration, BoundSql source, String countSql) {
    BoundSql target =
        new BoundSql(
            configuration,
            countSql,
            new ArrayList<>(source.getParameterMappings()),
  ...
  public static BoundSql forPage(Configuration configuration, BoundSql source, PageSql pageSql) {
    ...
    for (PageParameter parameter : pageSql.parameters()) {
      mappings.add(
          new ParameterMapping.Builder(configuration, parameter.name(), parameter.javaType())
              .build());
    }

    BoundSql target =
        new BoundSql(configuration, pageSql.sql(), mappings, source.getParameterObject());
