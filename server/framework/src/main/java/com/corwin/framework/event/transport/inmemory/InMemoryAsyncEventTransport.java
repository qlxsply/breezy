...
public class InMemoryAsyncEventTransport implements AsyncEventTransport {
    ...
    private static final Comparator<DelayedDispatchTask> DELAY_TASK_COMPARATOR = Comparator
            .comparingLong(DelayedDispatchTask::deliverAtMillis)
    ...
    public InMemoryAsyncEventTransport(InMemoryDispatchExecutor executor, ConsumeContextBinder contextBinder) {
    ...
    private void submitDelivery(SubscriptionDescriptor subscription, AsyncEventEnvelope<?> envelope) {
        ...
        delayQueue.offer(new DelayedDispatchTask(subscription, envelope, envelope.deliverAtMillis(),
     ...
     */
    private void invokeSubscriber(SubscriptionDescriptor subscription, AsyncEventEnvelope<?> envelope) {
        try (ConsumeContextScope ignored = contextBinder.bind(envelope.ctxSnapshot())) {
            subscription.invoker().invoke(envelope);
        } catch (Exception ex) {
            log.warn("Async event consume failed. eventId={}, eventType={}, subscriptionId={}, group={}, subscriber={}",
                    envelope.eventId(), envelope.eventType(), subscription.subscriptionId(),
                    subscription.consumerGroup(), subscription.subscriberName(), ex);
        }
    }
