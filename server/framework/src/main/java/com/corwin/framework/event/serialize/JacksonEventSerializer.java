 ...
 *
 * <p>Uses a two-phase deserialisation strategy: first reads the envelope shell and the generic
 * payload structure, then resolves the concrete event class via {@code eventType} and restores the
 * typed payload. This preserves type information without requiring manual per-type binding
 * configuration.
 *
...
public class JacksonEventSerializer implements EventSerializer {
  ...
  public byte[] serialize(AsyncEventEnvelope<?> envelope) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "Serialize async event failed. eventType=" + envelope.eventType(), ex);
    }
  }
  ...
  public AsyncEventEnvelope<?> deserialize(byte[] bytes) {
    ...
    if (!AsyncEvent.class.isAssignableFrom(payloadClass)) {
      throw new IllegalStateException(
          "Event payload class is not AsyncEvent: " + payloadClass.getName());
    }
    Object payload;
    try {
    ...
    return new AsyncEventEnvelope<>(
        ...
        serialized.occurredAtMillis(),
        deliverAtMillis,
        serialized.ctxSnapshot(),
        (AsyncEvent) payload,
        serialized.source(),
        serialized.producerService(),
        serialized.partitionKey(),
