...
public class ErrorCodeTest {
  ...
  void should_not_have_duplicate_codes_and_all_codes_should_be_in_range() {
    Class[] errorEnums =
        new Class[] {
          BaseError.class, DatabaseSourceError.class, JsonFmtError.class, ReminderError.class
        };
    ...
    for (Class<? extends ErrorCode> errorEnumClass : errorEnums) {
      ...
      for (ErrorCode errorCode : values) {
        ...
        assertTrue(
            errorCode.getRange().contains(code),
            owner
                + " code ["
                + code
                + "] is out of range ["
                + errorCode.getRange().getStart()
                + "-"
                + errorCode.getRange().getEnd()
                + "]");
  ...
  void should_not_have_overlapped_ranges() {
    ...
    for (int i = 0; i < ranges.length; i++) {
      for (int j = i + 1; j < ranges.length; j++) {
        ...
        boolean overlapped =
            a.getStart().compareTo(b.getEnd()) <= 0 && b.getStart().compareTo(a.getEnd()) <= 0;

        assertFalse(
            overlapped,
            "Error code range overlapped: ["
                + a.getName()
                + "] "
                + a.getStart()
                + "-"
