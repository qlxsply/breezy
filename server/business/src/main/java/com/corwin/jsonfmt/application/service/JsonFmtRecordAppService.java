...
public class JsonFmtRecordAppService {
  ...
  public List<JsonFmtRecordListItemView> list(Long userId, String keyword) {
    ...
    List<JsonFmtRecord> records =
        normalizedKeyword == null
            ? recordRepository.findByUserIdOrderByOrderNoAsc(userId)
            : recordRepository.findByUserIdAndNameContainingIgnoreCaseOrderByOrderNoAsc(
  ...
  public void rename(Long userId, JsonFmtRecordRenameCommand command) {
    ...
    JsonFmtRecord record =
        recordRepository
            .findByIdAndUserId(command.id(), userId)
            .orElseThrow(() -> new BizException(JsonFmtError.JSONFMT_RECORD_NOT_FOUND));
    record.rename(name);
    recordRepository.save(record);
  }
  ...
  private JsonFmtRecord createAsTop(JsonFmtRecord record) {
    List<JsonFmtRecord> existing =
        recordRepository.findByUserIdOrderByOrderNoAsc(record.getUserId());
    if (!existing.isEmpty()) {
  ...
  private String generateDefaultName(Long userId) {
    ...
    long sequence =
        recordRepository.countByUserIdAndCreatedAtGreaterThanEqualAndCreatedAtLessThan(
  ...
  private String createContentFile(String content) {
    ...
    return fileService.createInternalFile(
        fileName,
  ...
  private JsonFmtRecordListItemView toListItemView(JsonFmtRecord record) {
    return new JsonFmtRecordListItemView(
        record.getId(), record.getName(), record.getOrderNo(), record.getUpdatedAt());
  }

  private JsonFmtRecordDetailView toDetailView(JsonFmtRecord record) {
    return new JsonFmtRecordDetailView(
        record.getId(),
        record.getName(),
        resolveContent(record),
        record.getOrderNo(),
        record.getCreatedAt(),
        record.getUpdatedAt());
  }
