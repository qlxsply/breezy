...
public final class JsonFmtConfigSpecs {
    ...
    public static final ConfigSpec<StoragePolicy> STORAGE_POLICY = new SimpleConfigSpec<>(
            new ConfigKey("jsonfmt.storage.policy"), "jsonfmt", "storage", "JSON 内容存储策略",
            "JSON 格式化记录的内容存储策略", StoragePolicy.class, new StoragePolicy(61_440L), 1, 10,
            ConfigActivationPolicy.DYNAMIC, ConfigEditPolicy.ADMIN_EDITABLE, ConfigInvalidValuePolicy.USE_DEFAULT,
            List.of(new ConfigFieldSpec("contentFileThresholdBytes", "内容转文件阈值", "内容达到该字节数时转为文件存储",
                    ConfigFieldType.LONG, true, false, false, 10, "61440", BigDecimal.ONE, null, null, null,
                    List.of())), "default", JsonFmtConfigSpecs::validateStoragePolicy,
    ...
    private static List<ConfigViolation> validateStoragePolicy(StoragePolicy value) {
        return value != null && value.contentFileThresholdBytes() > 0 ? List.of() : List.of(
