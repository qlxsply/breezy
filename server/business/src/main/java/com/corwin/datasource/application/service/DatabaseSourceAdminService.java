...
public class DatabaseSourceAdminService {
    ...
    private static final Set<DatabaseType> USER_SUPPORTED_DB_TYPES = EnumSet.of(DatabaseType.MYSQL,
    ...
    public Long upsertDatabaseSource(UpsertConnectionCommand cmd) {
        ...
        if (normalized.id() == null) {
            ...
            String passwordEnc = secretCodec.encode(
                    Objects.requireNonNull(normalized.passwordRaw(), "passwordRaw required"));
            databaseSource = DatabaseSource.createUser(normalized.name(), normalized.dbType(), normalized.jdbcUrl(),
                    normalized.username(), passwordEnc, normalized.remarkCustom(), normalized.authMode(),
                    normalized.connectMode(), normalized.host(), normalized.port(), normalized.databaseName(),
                    normalized.serviceName(), normalized.sid(), normalized.driverClassName(), normalized.extraParams());
        } else {
            databaseSource = databaseSourceRepo.findById(normalized.id())
                    .orElseThrow(() -> new IllegalArgumentException("DataSource not found: " + normalized.id()));
            if (databaseSource.isAppSource()) {
                ...
                databaseSource.updateUserConfig(normalized.name(), normalized.dbType(), normalized.jdbcUrl(),
                        normalized.username(), passwordEnc, normalized.remarkCustom(), normalized.authMode(),
                        normalized.connectMode(), normalized.host(), normalized.port(), normalized.databaseName(),
                        normalized.serviceName(), normalized.sid(), normalized.driverClassName(),
    ...
    public void testDatabaseSource(Long id) {
        DatabaseSource databaseSource = databaseSourceRepo.findById(id)
    ...
    public TestConnectionView testDatabaseSourceConfig(UpsertConnectionCommand cmd) {
        UpsertConnectionCommand normalized = normalizeCommand(cmd);
        if (normalized.id() != null) {
            DatabaseSource existing = databaseSourceRepo.findById(normalized.id())
                    .orElseThrow(() -> new IllegalArgumentException("DataSource not found: " + normalized.id()));
        ...
        DatabaseSource databaseSource = DatabaseSource.createUser(normalized.name(), normalized.dbType(),
                normalized.jdbcUrl(), normalized.username(), passwordEnc, normalized.remarkCustom(),
                normalized.authMode(), normalized.connectMode(), normalized.host(), normalized.port(),
                normalized.databaseName(), normalized.serviceName(), normalized.sid(), normalized.driverClassName(),
                normalized.extraParams());
        try {
    ...
    public DatabaseSource getDatabaseSource(Long id) {
        return databaseSourceRepo.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("DataSource not found: " + id));
    }

    public DatabaseSourceConnectionInfoView getDatabaseSourceConnectionInfo(Long id) {
        DatabaseSource source = databaseSourceRepo.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("DataSource not found: " + id));
        String passwordRaw = resolveDatabaseSourcePasswordRaw(source);
        return new DatabaseSourceConnectionInfoView(source.getId(), source.getName(), source.getDbType(),
                source.getAuthMode(), source.getConnectMode(), source.getHost(), source.getPort(),
                source.getDatabaseName(), source.getServiceName(), source.getSid(), source.getDriverClassName(),
                source.getJdbcUrl(), source.getUsername(), passwordRaw, source.getRemarkCustom(),
    ...
    public List<DatabaseSourceSimpleView> listDatabaseSourcesSimple() {
        return databaseSourceRepo.findAll().stream()
                .map(source -> new DatabaseSourceSimpleView(source.getId(), source.getName(), source.getDbType(),
                        source.getUsername(), source.getStatus(), source.getSourceType())).toList();
    }

    // --- 数据库管理 ---

    public List<String> listAvailableDatabases(Long dataSourceId) {
        DatabaseSource source = databaseSourceRepo.findById(dataSourceId)
                .orElseThrow(() -> new IllegalArgumentException("DataSource not found: " + dataSourceId));
        return metadataExtractor.listDatabases(source);
    }

    @Transactional
    public Long createDatabaseSchema(Long dataSourceId, String databaseName, String alias, String remark) {
        ...
        DatabaseSchema schema = DatabaseSchema.create(dataSourceId, normalizedDatabaseName, normalizedAlias,
                normalizedRemark);
        schema.updateMetadata(baseInfo.productName(), baseInfo.productVersion(), baseInfo.driverName(),
                baseInfo.driverVersion(), baseInfo.fetchedAt());
    ...
    public void refreshDatabaseSchemaMetadata(Long managedDbId) {
        DatabaseSchema schema = databaseSchemaRepo.findById(managedDbId)
                .orElseThrow(() -> new IllegalArgumentException("Managed database not found: " + managedDbId));
        if (schema.getDataSourceId() == null) {
            throw new IllegalStateException("DataSource not configured");
        }
        DatabaseSource source = databaseSourceRepo.findById(schema.getDataSourceId())
                .orElseThrow(() -> new IllegalArgumentException("DataSource not found: " + schema.getDataSourceId()));

        MetadataExtractResult r = metadataExtractor.extract(source, schema.getDatabaseName());

        // 更新数据库层级元数据
        schema.updateMetadata(r.baseInfo().productName(), r.baseInfo().productVersion(), r.baseInfo().driverName(),
                r.baseInfo().driverVersion(), r.baseInfo().fetchedAt());
        databaseSchemaRepo.save(schema);
        ...
        for (MetadataExtractResult.TableInfo t : r.tables()) {
            DatabaseTable nt = DatabaseTable.create(source.getId(), schema.getId(), t.catalog(), t.schema(), t.name(),
                    t.type(), t.remarkDb());
        ...
        for (MetadataExtractResult.ColumnInfo col : r.columns()) {
            ...
            DatabaseColumn nc = DatabaseColumn.create(source.getId(), schema.getId(), tableId, col.columnName(),
                    col.typeName(), col.jdbcType(), col.columnSize(), col.decimalDigits(), col.nullable(),
                    col.ordinalPosition(), col.defaultValue(), col.remarkDb());
    ...
    public List<DatabaseSchema> listDatabaseSchemas(Long dataSourceId, boolean unboundOnly, String sortBy,
    ...
    public void updateDatabaseSchemaInfo(Long id, String alias, String remark) {
        DatabaseSchema schema = databaseSchemaRepo.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Managed database not found: " + id));
        schema.updateBasicInfo(alias, remark);
        databaseSchemaRepo.save(schema);
    }

    private Comparator<DatabaseSchema> buildDatabaseSchemaComparator(String sortBy, String sortDirection) {
        String normalizedSortBy = sortBy == null ? "DATA_SOURCE" : sortBy.trim().toUpperCase(Locale.ROOT);
        String normalizedSortDirection = sortDirection == null ? "ASC" : sortDirection.trim().toUpperCase(Locale.ROOT);

        Comparator<DatabaseSchema> comparator;
        if ("CREATED_AT".equals(normalizedSortBy)) {
            comparator = Comparator.comparing(this::resolveCreatedTime, Comparator.nullsLast(Comparator.naturalOrder()))
                    .thenComparing(DatabaseSchema::getId, Comparator.nullsLast(Comparator.naturalOrder()));
        } else {
            comparator = Comparator.comparing(DatabaseSchema::getDataSourceId,
                            Comparator.nullsFirst(Comparator.naturalOrder()))
                    .thenComparing(DatabaseSchema::getDatabaseName, Comparator.nullsLast(String.CASE_INSENSITIVE_ORDER))
                    .thenComparing(DatabaseSchema::getId, Comparator.nullsLast(Comparator.naturalOrder()));
        }
    ...
    public DatabaseSchema getDatabaseSchema(Long id) {
        return databaseSchemaRepo.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Managed database not found: " + id));
    }
    ...
    public DatabaseSchemaBasicInfoView getDatabaseSchemaBasicInfo(Long id) {
        ...
        String passwordRaw = resolveDatabaseSourcePasswordRaw(source);
        return new DatabaseSchemaBasicInfoView(source.getName(), source.getDbType(), source.getJdbcUrl(),
                source.getUsername(), passwordRaw);
    }

    private String resolveDatabaseSourcePasswordRaw(DatabaseSource source) {
        if (source.isAppSource()) {
            if (StrUtil.isNotBlank(source.getAppDsKey())) {
                String appPassword = appDataSourceProvider.findPasswordByKey(source.getAppDsKey()).orElse(null);
    ...
    private UpsertConnectionCommand normalizeCommand(UpsertConnectionCommand cmd) {
        ...
        if (connectMode == null) {
            connectMode = cmd.dbType() == DatabaseType.ORACLE ? ConnectMode.SERVICE_NAME : ConnectMode.HOST_PORT;
        }
        ...
        return new UpsertConnectionCommand(cmd.id(), cmd.name(), cmd.dbType(), authMode, connectMode,
                StrUtil.trimToNull(cmd.host()), port, StrUtil.trimToNull(cmd.databaseName()),
                StrUtil.trimToNull(cmd.serviceName()), StrUtil.trimToNull(cmd.sid()), driverClassName, cmd.jdbcUrl(),
                cmd.username(), cmd.passwordRaw(), cmd.defaultSchema(), cmd.remarkCustom(),
    ...
    private void assertDatabaseSchemaNotDuplicated(Long dataSourceId, String databaseName, Long excludedManagedDbId) {
        databaseSchemaRepo.findByDataSourceIdAndDatabaseName(dataSourceId, databaseName)
    ...
    private boolean isDatabaseSchemaUniqueConflict(DataIntegrityViolationException ex) {
        ...
        return msg.contains("db_schema") && (msg.contains("duplicate") || msg.contains("unique") || msg.contains(
                "constraint"));
    }
    ...
    public void updateTableInfo(Long tableId, String alias, String remark) {
        DatabaseTable t = tableRepo.findById(tableId)
    ...
    public void updateColumnInfo(Long columnId, String alias, String remark) {
        DatabaseColumn c = columnRepo.findById(columnId)
                .orElseThrow(() -> new IllegalArgumentException("Column not found: " + columnId));
        c.updateBasicInfo(alias, remark);
        columnRepo.save(c);
    }

    public PageData<DatabaseTable> pageTables(Long managedDbId, String schema, String nameLike, String tableType,
            PageSpec spec) {
        if (managedDbId == null) {
