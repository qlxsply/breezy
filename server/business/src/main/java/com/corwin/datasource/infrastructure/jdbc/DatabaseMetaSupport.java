...
/**
 * 处理跨库差异：驱动类、remarks 支持等。 注意：是否支持 remarks 取决于驱动与连接参数，且不是所有数据库/驱动都能通过 DatabaseMetaData 拿到备注。
 *
...
public final class DatabaseMetaSupport {
  ...
  /**
   * 选择用于抓取的 schema/catalog（跨库尽量通用）： - 优先使用连接配置 defaultSchema - 否则使用 Connection.getSchema() /
   * getCatalog() 或 meta.getUserName()（Oracle 常用）
   */
