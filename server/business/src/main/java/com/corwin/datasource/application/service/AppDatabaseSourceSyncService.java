...
public class AppDatabaseSourceSyncService {
  ...
  private void syncSingle(String beanName, DataSource rawDataSource, Instant startupTime) {
    ...
    DatabaseSource existing =
        databaseSourceRepo
            .findByAppDsKey(beanName)
            .orElseGet(
                () ->
                    databaseSourceRepo
                        .findByDbTypeAndJdbcUrlAndUsername(
                            spec.dbType(), spec.jdbcUrl(), spec.username())
                        .filter(DatabaseSource::isAppSource)
                        .orElse(null));
    if (existing != null && isSameConnection(existing, spec)) {
      existing.updateAppConfig(
          existing.getName(), spec.dbType(), spec.jdbcUrl(), spec.username(), beanName);
    ...
    DatabaseSource created =
        DatabaseSource.createApp(
            name, spec.dbType(), spec.jdbcUrl(), spec.username(), beanName, null);
  ...
  private void cleanupMissingAppSources(Set<String> beanKeys) {
    List<DatabaseSource> appSources =
        databaseSourceRepo.findAll().stream().filter(DatabaseSource::isAppSource).toList();
  ...
  private boolean isSameConnection(DatabaseSource existing, AppDataSourceSpec spec) {
    return existing.getDbType() == spec.dbType()
        && Objects.equals(existing.getJdbcUrl(), spec.jdbcUrl())
        && Objects.equals(existing.getUsername(), spec.username());
  }
  ...
  private AppDataSourceSpec resolveSpec(String beanName, DataSource dataSource) {
    ...
    if (dbType == null) {
      log.warn(
          "无法识别数据库类型，已跳过：beanName={}, jdbcUrl={}, chain={}",
          beanName,
          jdbcUrl,
          unwrapResult.chain());
      return null;
  ...
  private String resolveJdbcUrl(Object dataSource) {
    ...
    return readStringProperty(dataSource, "getJdbcUrl")
        .orElseGet(() -> readStringProperty(dataSource, "getUrl").orElse(null));
  }
  ...
  private String resolveUsername(Object dataSource) {
    ...
    return readStringProperty(dataSource, "getUsername")
        .orElseGet(() -> readStringProperty(dataSource, "getUser").orElse(null));
  }
  ...
  private record AppDataSourceSpec(
      String beanName, String jdbcUrl, String username, DatabaseType dbType) {}

  private record UnwrapResult(DataSource dataSource, String chain) {}
}
