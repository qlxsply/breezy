...
public class DatabaseSourceAdminController {
  ...
  public ApiResponse<String> upsertDatabaseSource(@RequestBody UpsertConnectionReq req) {
    UpsertConnectionCommand cmd =
        new UpsertConnectionCommand(
            req.getId(),
            req.getName(),
            req.getDbType(),
            req.getAuthMode(),
  ...
  @PostMapping("/page")
  public ApiResponse<PageResult<ConnectionRes>> pageDatabaseSources(
      @RequestBody ConnectionPageReq req) {
    var page =
        service.pageDatabaseSources(
            req.dbType(), req.nameLike(), PageSpecFactory.of(req.page(), req.sort()));
  ...
  public ApiResponse<DatabaseSourceConnectionInfoRes> getDatabaseSourceConnectionInfo(
      @PathVariable Long id) {
    var view = service.getDatabaseSourceConnectionInfo(id);
    return ApiResponse.ok(
        new DatabaseSourceConnectionInfoRes(
            view.id(),
            view.name(),
            view.dbType(),
            view.authMode(),
  ...
  public ApiResponse<List<DatabaseSourceSimpleRes>> listDatabaseSourcesSimple() {
    return ApiResponse.ok(
        service.listDatabaseSourcesSimple().stream()
            .map(DatabaseSourceAdminController::toDto)
            .toList());
  }
  ...
  public ApiResponse<List<CatalogRes>> listDatabaseSchemas(@RequestBody DatabaseSchemaListReq req) {
    DatabaseSchemaListReq resolved =
        req == null ? new DatabaseSchemaListReq(null, null, null, null) : req;
    boolean unboundOnly = Boolean.TRUE.equals(resolved.unboundOnly());
    String sortBy =
        resolved.sortBy() == null || resolved.sortBy().isBlank()
            ? "DATA_SOURCE"
            : resolved.sortBy();
    String sortDirection =
        resolved.sortDirection() == null || resolved.sortDirection().isBlank()
            ? "ASC"
            : resolved.sortDirection();
    return ApiResponse.ok(
        service
            .listDatabaseSchemas(resolved.dataSourceId(), unboundOnly, sortBy, sortDirection)
            .stream()
            .map(DatabaseSourceAdminController::toDto)
            .toList());
  }
  ...
  public ApiResponse<DatabaseSchemaBasicInfoRes> getDatabaseSchemaBasicInfo(
      @PathVariable("id") Long id) {
    var view = service.getDatabaseSchemaBasicInfo(id);
    return ApiResponse.ok(
        new DatabaseSchemaBasicInfoRes(
            view.dataSourceName(),
            view.dbType(),
            view.jdbcUrl(),
            view.username(),
  ...
  public ApiResponse<PageResult<ColumnRes>> pageColumns(@RequestBody ColumnPageReq req) {
    var page =
        service.pageColumns(
            req.tableId(), req.nameLike(), PageSpecFactory.of(req.page(), req.sort()));
    return ApiResponse.ok(PageResult.of(page, DatabaseSourceAdminController::toDto));
  }
  ...
  public ApiResponse<List<TableRes>> listAllTables(@PathVariable("id") Long id) {
    return ApiResponse.ok(
        service.listAllTables(id).stream().map(DatabaseSourceAdminController::toDto).toList());
  }

  @GetMapping("/tables/{id}/columns/all")
  public ApiResponse<List<ColumnRes>> listAllColumns(@PathVariable("id") Long id) {
    return ApiResponse.ok(
        service.listAllColumns(id).stream().map(DatabaseSourceAdminController::toDto).toList());
  }

  @PutMapping("/tables/{id}/info")
  public ApiResponse<Object> updateTableInfo(
      @PathVariable("id") Long id, @RequestBody UpdateInfoReq req) {
    service.updateTableInfo(id, req.getAlias(), req.getRemarkCustom());
    return ApiResponse.ok();
  }

  @PutMapping("/columns/{id}/info")
  public ApiResponse<Object> updateColumnInfo(
      @PathVariable("id") Long id, @RequestBody UpdateInfoReq req) {
  ...
  public ApiResponse<Object> rebindDatabaseSchema(
      @PathVariable("id") Long id, @RequestBody DatabaseSchemaRebindReq req) {
    service.rebindDatabaseSchema(id, req.getDataSourceId());
    return ApiResponse.ok();
  }

  @PutMapping("/database-schemas/{id}/info")
  public ApiResponse<Object> updateDatabaseSchemaInfo(
      @PathVariable("id") Long id, @RequestBody UpdateInfoReq req) {
  ...
  private static DatabaseSourceSimpleRes toDto(DatabaseSourceSimpleView source) {
    return new DatabaseSourceSimpleRes(
        source.id(),
        source.name(),
        source.dbType(),
        source.username(),
