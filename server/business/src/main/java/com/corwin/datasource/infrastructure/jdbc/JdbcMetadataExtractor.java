...
public class JdbcMetadataExtractor implements MetadataExtractor {
    ...
    public List<String> listDatabases(DatabaseSource dataSource) {
        ...
        try (Connection conn = connectionFactory.openConnection(dataSource)) {
            DatabaseMetaData meta = conn.getMetaData();
            if (dataSource.getDbType() == DatabaseType.MYSQL || dataSource.getDbType() == DatabaseType.SQLSERVER) {
                // 使用 Catalogs
    ...
    public MetadataExtractResult extract(DatabaseSource dataSource, String databaseName) {
        try (Connection conn = connectionFactory.openConnection(dataSource)) {
            ...
            MetadataExtractResult.BaseInfo baseInfo = new MetadataExtractResult.BaseInfo(
                    meta.getDatabaseProductName(),
                    meta.getDatabaseProductVersion(),
                    meta.getDriverName(),
                    meta.getDriverVersion(),
    ...
    private List<MetadataExtractResult.TableInfo> loadTables(DatabaseMetaData meta, String catalog,
        ...
        try (ResultSet rs = meta.getTables(catalog, schema, "%", types)) {
            while (rs.next()) {
                ...
                list.add(new MetadataExtractResult.TableInfo(tableCat, tableSchem, tableName, tableType, remarks));
            }
        }
        return list;
    }

    private List<MetadataExtractResult.ColumnInfo> loadColumns(DatabaseMetaData meta, String catalog, String schema,
        ...
        try (ResultSet rs = meta.getColumns(catalog, schema, table, "%")) {
            while (rs.next()) {
                ...
                Boolean nullable = nullableInt == null ? null : (nullableInt != DatabaseMetaData.columnNoNulls);
                String remarks = rs.getString("REMARKS");
                String defaultValue = rs.getString("COLUMN_DEF");
                Integer ordinalPosition = getIntOrNull(rs, "ORDINAL_POSITION");

                list.add(new MetadataExtractResult.ColumnInfo(tableCat, tableSchem, tableName, columnName, typeName,
                        dataType, columnSize, decimalDigits, nullable,
                        ordinalPosition, defaultValue, remarks));
            }
        }
        return list;
    }
