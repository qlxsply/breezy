...
public class LiquibaseEngine {
    ...
    public SnapshotResult generateSnapshot(Connection connection, Set<String> selectedTableNames,
            Set<String> selectedViewNames) {
        try {
            Database database = toDatabase(connection);
            SnapshotControl snapshotControl = createSnapshotControl(database, selectedTableNames, selectedViewNames);
            ...
            return new SnapshotResult(content, database.getShortName(), databaseVersion(database),
                    schemaName(database));
        } catch (Exception e) {
    ...
    public DdlResult generateDdlFromSnapshots(byte[] sourceSnapshotContent, byte[] targetSnapshotContent,
            String sourceQualifierHint, String targetQualifierHint) {
        try {
            DatabaseSnapshot targetSnapshot = parseSnapshot(targetSnapshotContent, "target.snapshot.yaml");
            ...
            return new DdlResult(sql.getBytes(StandardCharsets.UTF_8), sourceDatabase.getShortName(),
                    databaseVersion(sourceDatabase), schemaName(sourceDatabase));
        } catch (Exception e) {
    ...
    private SnapshotContext buildSnapshotContext(Connection connection, Set<String> selectedTableNames,
            Set<String> selectedViewNames) throws Exception {
        Database database = toDatabase(connection);
        SnapshotControl snapshotControl = createSnapshotControl(database, selectedTableNames, selectedViewNames);
        DatabaseSnapshot snapshot = SnapshotGeneratorFactory.getInstance()
                .createSnapshot(database.getDefaultSchema(), database, snapshotControl);
        ...
        DiffResult diffResult = DiffGeneratorFactory.getInstance()
                .compare(snapshot, emptySnapshot, new CompareControl());
    ...
    private SnapshotControl createSnapshotControl(Database database, Set<String> selectedTableNames,
            Set<String> selectedViewNames) {
        ObjectChangeFilter objectChangeFilter = buildObjectChangeFilter(selectedTableNames, selectedViewNames);
    ...
    private ObjectChangeFilter buildObjectChangeFilter(Set<String> selectedTableNames, Set<String> selectedViewNames) {
        ...
        return new StandardObjectChangeFilter(StandardObjectChangeFilter.FilterType.INCLUDE, filterBuilder.toString());
    }
    ...
    private String toObjectNamePattern(Set<String> names) {
        ...
        String merged = names.stream().map(this::normalizeObjectName).filter(item -> item != null && !item.isEmpty())
    ...
    private DatabaseSnapshot buildSourceSnapshot(byte[] sourceSnapshotContent, DatabaseSnapshot targetSnapshot) throws
            DatabaseException, LiquibaseException {
        if (sourceSnapshotContent == null || sourceSnapshotContent.length == 0) {
            SnapshotControl snapshotControl = targetSnapshot.getSnapshotControl() == null ? new SnapshotControl(
                    targetSnapshot.getDatabase()) : targetSnapshot.getSnapshotControl();
    ...
    private String renderChangeLogXml(DiffResult diffResult) throws ParserConfigurationException, IOException,
            DatabaseException {
    ...
    private DiffOutputControl buildOutputControlForSnapshots(QualifierMode qualifierMode, String sourceQualifier,
        ...
        switch (qualifierMode) {
            ...
            case AUTO -> {
                boolean sameQualifier = Objects.equals(normalizeQualifier(sourceQualifier),
    ...
    private String alignSqlQualifierToSource(String sql, String sourceQualifier, String targetQualifier) {
        ...
        Pattern plainQualifierPattern = Pattern.compile("(?i)\\b" + Pattern.quote(targetQualifier) + "\\.");
        result = plainQualifierPattern.matcher(result).replaceAll(Matcher.quoteReplacement(sourceQualifier + "."));
        return result;
    }
    ...
    private static final class MemoryResourceAccessor implements ResourceAccessor {
        ...
        private MemoryResourceAccessor(Map<String, byte[]> contents) {
            this.resources = contents.entrySet().stream().collect(
                    Collectors.toMap(Map.Entry::getKey, entry -> new MemoryResource(entry.getKey(), entry.getValue())));
        }
    ...
    private static final class MemoryResource implements Resource {
        ...
        @Override
        public OutputStream openOutputStream(liquibase.resource.OpenOptions openOptions) throws IOException {
