...
public class SchemaForgeController {
  ...
  public ApiResponse<String> createSnapshot(@RequestBody CreateSchemaSnapshotReq req) {
    String id =
        schemaSnapshotAppService.createSnapshot(
            new CreateSchemaSnapshotCommand(
                req.managedDatabaseId(), req.name(), req.remark(), req.selectedObjectIds()));
  ...
  public ApiResponse<List<SnapshotSelectableObjectRes>> listSnapshotSelectableObjects(
    ...
    return ApiResponse.ok(
        schemaSnapshotAppService.listSnapshotSelectableObjects(managedDatabaseId).stream()
            .map(SchemaForgeController::toSnapshotSelectableObjectRes)
            .toList());
  }

  @PostMapping("/snapshots/page")
  public ApiResponse<PageResult<SchemaSnapshotRes>> pageSnapshots(
      @RequestBody SchemaSnapshotPageReq req) {
    return ApiResponse.ok(
        schemaSnapshotAppService.pageQuery(
            req.managedDatabaseId(), req.nameLike(), PageSpecFactory.of(req.page(), req.sort())));
  }
  ...
  public ApiResponse<Object> updateSnapshot(
      @PathVariable String id, @RequestBody UpdateSchemaSnapshotReq req) {
    schemaSnapshotAppService.updateInfo(
        new UpdateSchemaSnapshotInfoCommand(id, req.name(), req.remark()));
    return ApiResponse.ok();
  }
  ...
  public ApiResponse<String> createDdl(@RequestBody CreateSchemaDdlReq req) {
    String id =
        schemaDdlAppService.createDdl(
            new CreateSchemaDdlCommand(
                req.sourceSnapshotId(), req.targetSnapshotId(), req.name(), req.remark()));
    return ApiResponse.ok(id);
  }
  ...
  public ApiResponse<PageResult<SchemaDdlRes>> pageDdls(@RequestBody SchemaDdlPageReq req) {
    return ApiResponse.ok(
        schemaDdlAppService.pageQuery(
            req.managedDatabaseId(), req.nameLike(), PageSpecFactory.of(req.page(), req.sort())));
  }

  @PutMapping("/ddls/{id}")
  public ApiResponse<Object> updateDdl(
      @PathVariable String id, @RequestBody UpdateSchemaDdlReq req) {
  ...
  private static SnapshotSelectableObjectRes toSnapshotSelectableObjectRes(
      SnapshotSelectableObjectView view) {
    return new SnapshotSelectableObjectRes(
        view.id(), view.tableName(), view.tableSchema(), view.tableType(), view.alias());
  }
}
