...
public class TodoController {
    ...
    public ApiResponse<Long> create(@RequestBody TodoCreateReq req) {
        Long id = service.create(
                new TodoSaveCommand(req.content(), req.dueTime(), req.note(), req.contentAttachmentFileIds(),
    ...
    public ApiResponse<Object> update(@PathVariable Long id, @RequestBody TodoUpdateReq req) {
        service.update(id, new TodoSaveCommand(req.content(), req.dueTime(), req.note(), req.contentAttachmentFileIds(),
                req.completionAttachmentFileIds()));
    ...
    public ApiResponse<PageResult<TodoRes>> page(@RequestBody TodoPageReq req) {
        var page = service.page(req.status(), req.contentLike(), PageSpecFactory.of(req.page(), req.sort()));
    ...
    public ApiResponse<Object> reorder(@RequestBody TodoReorderReq req) {
        List<TodoReorderCommand.GroupCommand> groups = req.groups() == null ? List.of() : req.groups().stream()
                .map(group -> new TodoReorderCommand.GroupCommand(group.status(), group.orderedIds())).toList();
    ...
    public ApiResponse<Object> complete(@PathVariable Long id, @RequestBody TodoCompleteReq req) {
        service.complete(id, new TodoCompleteCommand(req.completionNote(), req.completionAttachmentFileIds()));
        return ApiResponse.ok();
    }
    ...
    private static TodoRes toDto(TodoView view) {
        return new TodoRes(view.id(), view.content(), view.dueTime(), view.status(), view.note(),
                view.contentAttachmentFileIds(), view.completionAttachmentFileIds(), view.sortNo(),
                view.completionNote(), view.completedAt(), view.createdAt());
    }
    ...
    private static TodoDailyDetailRes toDailyDetailRes(TodoDailyDetailView view) {
        return new TodoDailyDetailRes(view.date(), view.createdItems().stream().map(TodoController::toDto).toList(),
