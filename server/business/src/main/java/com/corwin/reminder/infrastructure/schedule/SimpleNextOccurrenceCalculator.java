...
public class SimpleNextOccurrenceCalculator implements NextOccurrenceCalculator {
    ...
    public Instant nextStart(ScheduleEvent event, Instant fromExclusive) {
        ...
        ZonedDateTime from = ZonedDateTime.ofInstant(fromExclusive.plusNanos(1), eventZone); // strict > exclusive
        ...
        switch (rule.getFrequency()) {
            ...
            case YEARLY ->
                    next = nextYearly(start, from, rule.getInterval(), rule.getMonthOfYear(), rule.getDayOfMonth());
            default -> next = null;
        }
    ...
    private ZonedDateTime nextDaily(ZonedDateTime start, ZonedDateTime from, int intervalDays) {
        ...
        ZonedDateTime candidate = from.withHour(start.getHour()).withMinute(start.getMinute())
                .withSecond(start.getSecond()).withNano(start.getNano());
        ...
        // 对齐到 interval
        long daysBetween = Duration.between(start.toLocalDate().atStartOfDay(start.getZone()),
                candidate.toLocalDate().atStartOfDay(candidate.getZone())).toDays();
    ...
    private ZonedDateTime nextWeekly(ZonedDateTime start, ZonedDateTime from, int intervalWeeks, Set<DayOfWeek> days) {
        if (days == null || days.isEmpty()) {
            return null;
        }

        // 从 from 的下一分钟（保持 start 的时分秒）开始找最近满足 dayOfWeek 的日期
        ZonedDateTime base = from.withHour(start.getHour()).withMinute(start.getMinute()).withSecond(start.getSecond())
    ...
    private long weeksBetweenStartWeek(ZonedDateTime start, ZonedDateTime candidate) {
        ...
        return Duration.between(s.atStartOfDay(start.getZone()), c.atStartOfDay(candidate.getZone())).toDays() / 7;
    }

    private ZonedDateTime nextMonthly(ZonedDateTime start, ZonedDateTime from, int intervalMonths, Integer dayOfMonth) {
        if (dayOfMonth == null) {
            return null;
        }
    ...
    private boolean monthAligned(ZonedDateTime start, YearMonth candYm, int intervalMonths) {
        ...
        long monthsBetween =
                (candYm.getYear() - startYm.getYear()) * 12L + (candYm.getMonthValue() - startYm.getMonthValue());
        return monthsBetween >= 0 && monthsBetween % intervalMonths == 0;
    }

    private ZonedDateTime nextYearly(ZonedDateTime start, ZonedDateTime from, int intervalYears, Integer monthOfYear,
            Integer dayOfMonth) {
