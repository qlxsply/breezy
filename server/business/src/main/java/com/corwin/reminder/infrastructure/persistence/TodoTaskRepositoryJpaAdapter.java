...
public class TodoTaskRepositoryJpaAdapter implements TodoTaskRepository {
  ...
  @Override
  public PageData<TodoTask> findByStatusInAndRemindAtLessThanEqual(
  ...
  @Override
  public List<TodoTask> findByCreatedAtGreaterThanEqualAndCreatedAtLessThan(
  ...
  @Override
  public List<TodoTask> findByCompletedAtGreaterThanEqualAndCompletedAtLessThan(
      Instant startInclusive, Instant endExclusive) {
    return repo.findByCompletedAtGreaterThanEqualAndCompletedAtLessThan(
  ...
  private TodoTaskPageQuery normalizeQuery(TodoTaskPageQuery query) {
    ...
    List<TodoTaskStatus> statuses =
        query.statuses() == null
            ? List.of()
            : query.statuses().stream().filter(Objects::nonNull).toList();
